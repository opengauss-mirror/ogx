...
System Requirements:
- Rustup. Users may be able to use distro toolchains, but you won't get far without a proper Rust toolchain manager.
- Otherwise, same as the user requirements.

If you want to be ready to open a PR, you will want to run
```bash
git clone --branch develop "https://gitee.com/Nelson-He/ogx"
cd ogx
```
...
However, if there are any differences in `cargo ogx` since the last release, then
the first and most drastic difference in the developer environment vs. the user environment is that you will have to run
```bash
cargo install cargo-ogx --path ./cargo-ogx \
    --force \
    --locked # This forces usage of the repo's lockfile
cargo ogx init # This might take a while. Consider getting a drink.
```
...
- OGX tests PRs on rustfmt so please run `cargo fmt` before you submit
- Diffs in Cargo.lock should be checked in
- HOWEVER, diffs in the bindgen in `ogx-pg-sys/src/og*.rs` should **not** be checked in (this is a release task)
...
You do not need exceptional justification notes in your PR to justify a new dependency as your code will, in most cases, self-evidently justify the use of the dependency. OGX uses the normal Rust approach of using dependencies based on their ability to improve correctness and make features possible. It does not reimplement things already available in the Rust ecosystem unless the addition is trivial (do not add custom derives to save 5~10 lines of code in one site) or the ecosystem crates are not compatible with openGauss (unfortunately common for openGauss data types).

## Releases

On a new OGX release, *develop* will be merged to *master* via merge commit.
<!-- it's somewhat ambiguous whether we do this for stable or also "release candidate" releases -->

### Release Candidates AKA Betas
OGX prefers using `x.y.z-{alpha,beta}.n` format for naming release candidates,
...
- [ ] Run `./update-versions.sh "${NEW_RELEASE_VERSION}"`
    - This will update the visible bindings of `ogx-pg-sys/src/pg*.rs`
    - The visible bindings are for reference, [docs][ogx@docs.rs], and tools
    - Actual users of the library rebuild the bindings from scratch
- [ ] Run `./upgrade-deps.sh`
- [ ] Push the resulting diffs to *develop*
- [ ] Run `./publish.sh` to push the new version to [ogx@crates.io]
...
You agree that all code you submit in pull requests to https://github.com/tcdi/ogx/pulls
...
[ogx@crates.io]: https://crates.io/crates/ogxa
[ogx@docs.rs]: https://docs.rs/ogx/latest/ogx