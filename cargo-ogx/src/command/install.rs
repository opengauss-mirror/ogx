...
/// Install the extension from the current crate to the openGauss specified by whatever `pg_config` is currently on your $PATH
...
pub(crate) fn install_extension(
...

    {
        ...
        let so_name = if versioned_so {
            ...
            // note: versioned so-name format must agree with ogx-utils
