...
/// Compile/install extension to a ogx-managed openGauss instance and start psql
...
pub(crate) fn exec_gsql(pg_config: &PgConfig, dbname: &str, pgcli: bool) -> eyre::Result<()> {
    let mut command = Command::new(match pgcli {
        false => pg_config.gsql_path()?.into_os_string(),
