...
use crate::ogx_pg_sys_stub::OgxPgSysStub;
...
use ogx_pg_config::{get_target_dir, PgConfig, Ogx};
use ogx_utils::sql_entity_graph::{ControlFile, OgxSql, SqlGraphEntity};
            ...
            None => match self.pg_version {
                None => {
                    let pg_version = match self.pg_version {
                        Some(s) => s,
                        None => crate::manifest::default_og_version(&package_manifest)
                            .ok_or(eyre!("No provided `og$VERSION` flag."))?,
                    };
                    (Ogx::from_config()?.get(&pg_version)?.clone(), pg_version)
                }
                Some(pgver) => (Ogx::from_config()?.get(&pgver)?.clone(), pgver),
            },
            Some(config) => {
...
pub(crate) fn generate_schema(
    ...
    let flags = std::env::var("OGX_BUILD_FLAGS").unwrap_or_default();
    ...
    // Create stubbed `ogx_pg_sys` bindings for the generator to link with.
    let mut postmaster_stub_dir =
        Ogx::postmaster_stub_dir().wrap_err("couldn't get postmaster stub dir env")?;
    ...
    // Inspect the symbol table for a list of `__ogx_internals` we should have the generator call
    ...
    for export in lib_so_exports {
        ...
        if name.starts_with("__ogx_internals") {
    ...
    for func in &fns_to_call {
        if func.starts_with("__ogx_internals_schema_") {
            let schema = func
                .split("_")
                .skip(5)
                .next()
        ...
        } else if func.starts_with("__ogx_internals_fn_") {
            num_funcs += 1;
        } else if func.starts_with("__ogx_internals_trigger_") {
            num_triggers += 1;
        } else if func.starts_with("__ogx_internals_type_") {
            num_types += 1;
        } else if func.starts_with("__ogx_internals_enum_") {
            num_enums += 1;
        } else if func.starts_with("__ogx_internals_sql_") {
            num_sqls += 1;
        } else if func.starts_with("__ogx_internals_ord_") {
            num_ords += 1;
        } else if func.starts_with("__ogx_internals_hash_") {
            num_hashes += 1;
        } else if func.starts_with("__ogx_internals_aggregate_") {
            num_aggregates += 1;
        }
    }
    ...
    unsafe {
        ...
        let sql_mappings_symbol: libloading::os::unix::Symbol<
            unsafe extern "Rust" fn() -> ::ogx_utils::sql_entity_graph::RustToSqlMapping,
        > = lib
            .get("__ogx_sql_mappings".as_bytes())
            .expect(&format!("Couldn't call __ogx_sql_mappings"));
        sql_mapping = sql_mappings_symbol();

        let symbol: libloading::os::unix::Symbol<
            unsafe extern "Rust" fn() -> eyre::Result<ControlFile>,
        > = lib
            .get("__ogx_marker".as_bytes())
            .expect(&format!("Couldn't call __ogx_marker"));
    ...
    let ogx_sql =
        OgxSql::build(sql_mapping, entities.into_iter(), package_name.to_string(), versioned_so)
...
fn create_stub(
    ...
    tracing::debug!("Creating stub of appropriate openGauss symbols");
    OgxPgSysStub::from_symbols(&symbols_to_stub)?.write_to_file(&postmaster_stub_file)?;
