...
use ogx_pg_config::{get_target_dir, PgConfig, PgConfigSelector, Ogx};
            ...
            let pg_config = match pg_config {
                Err(error) => {
                    tracing::debug!(
                        ...
                        error = %error,
                        "Got invalid `og$VERSION` flag, assuming it is a testname"
                    );
...
pub fn test_extension(
    ...
    if features.features.iter().all(|f| f != "og_test") {
        features_arg += " og_test";
    }
    ...
    command
        ...
        .env("OGX_FEATURES", features_arg.clone())
        .env("OGX_NO_DEFAULT_FEATURES", if no_default_features_arg { "true" } else { "false" })
        .env("OGX_ALL_FEATURES", if features.all_features { "true" } else { "false" })
        .env("OGX_BUILD_PROFILE", profile.name())
        .env("OGX_NO_SCHEMA", if no_schema { "true" } else { "false" });
