...
/// `cargo` stub for `cargo-ogx` (you probably meant to run `cargo ogx`)
...
fn main() -> color_eyre::Result<()> {
    ...
    let filter_layer = match EnvFilter::try_from_default_env() {
        ...
        Err(_) => {
            ...
            let filter_layer =
                filter_layer.add_directive(format!("cargo_ogx={}", log_level).parse()?);
            let filter_layer = filter_layer.add_directive(format!("ogx={}", log_level).parse()?);
            let filter_layer =
                filter_layer.add_directive(format!("ogx_macros={}", log_level).parse()?);
            let filter_layer =
                filter_layer.add_directive(format!("ogx_tests={}", log_level).parse()?);
            let filter_layer =
                filter_layer.add_directive(format!("ogx_pg_sys={}", log_level).parse()?);
            let filter_layer =
                filter_layer.add_directive(format!("ogx_utils={}", log_level).parse()?);
            filter_layer
        }
    };
