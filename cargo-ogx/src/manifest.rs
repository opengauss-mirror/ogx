...
use eyre::eyre;
use ogx_pg_config::SUPPORTED_MAJOR_VERSIONS;
...
pub(crate) fn manifest_path(
    ...
    let manifest_path = if let Some(package_name) = package_name {
        ...
        let root = metadata.root_package().ok_or(eyre!(
            "`ogx` requires a root package in a workspace when `--package` is not specified."
        ))?;
...
pub(crate) fn default_og_version(manifest: &Manifest) -> Option<String> {
    ...
    for default_feature in default_features {
        for major_version in SUPPORTED_MAJOR_VERSIONS {
            let potential_feature = format!("og{}", major_version);
            if *default_feature == format!("og{}", major_version) {
            ...
            let default_features = default_features
                ...
                .filter(|default_feature| {
                    for supported_major in SUPPORTED_MAJOR_VERSIONS {
                        if **default_feature == format!("og{}", supported_major) {
                            return false;
                        }
                    }
                    true
