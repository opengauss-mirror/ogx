...
/// A utility structure which can generate 'stubs' of the bindings generated by `ogx-pg-sys`'s build script.
///
/// These stubs can be built.
///
/// For example, this is used by `cargo-ogx` and then `dlopen`'d before the extension for SQL generation.
pub struct OgxPgSysStub {
    stub_file: syn::File,
}
...
impl OgxPgSysStub {
    ...
    #[tracing::instrument(level = "error", skip_all, fields(ogx_pg_sys_stub = %ogx_pg_sys_stub.as_ref().display()))]
    pub fn write_to_file(&self, ogx_pg_sys_stub: impl AsRef<Path>) -> eyre::Result<()> {
        let ogx_pg_sys_stub = ogx_pg_sys_stub.as_ref();
        if let Some(parent) = ogx_pg_sys_stub.parent() {
            std::fs::create_dir_all(parent)?;
        }
        let mut output_file = File::create(ogx_pg_sys_stub)?;
