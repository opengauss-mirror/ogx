...
*/
use ogx::prelude::*;
use ogx::{check_for_interrupts, info, register_xact_callback, PgRelation, OgXactCallbackEvent};
use std::fs::File;
use std::io::Write;
...
#[og_extern]
fn random_abort() {
    register_xact_callback(OgXactCallbackEvent::PreCommit, || {
...
/// with `no_guard` we're telling ogx that we're positive this function
