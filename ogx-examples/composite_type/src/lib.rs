...
/** A guided example of composite types in `ogx`

Composite types in `ogx` are essentially macros that alias to a [`PgHeapTuple`][ogx::PgHeapTuple]
with one important difference:

During SQL generation, the composite type name provided to the [`ogx::composite_type`][ogx::composite_type]
macro is used as the type name, whereas [`PgHeapTuple`][ogx::PgHeapTuple] does not have a valid
Rust to SQL mapping.

A number of considerations are detailed in the [`ogx::composite_type`][ogx::composite_type] and
[`PgHeapTuple`][ogx::PgHeapTuple] documentation which may assist in productionalizing extensions
using composite types.
*/
use ogx::{opname, og_operator, prelude::*, Aggregate};

// All `ogx` extensions will do this:
ogx::pg_module_magic!();
...
```rust
const CAT_COMPOSITE_TYPE: PgHeapTuple<'static, AllocatedByRust> = ogx::composite_type!("Cat");
```
This is not correct! The [`#[ogx::og_extern]`][ogx::og_extern] macro would not be able to pick
up the required metadata for SQL generation. Instead, set the constant as a `&'static str` which
the [`ogx::composite_type!()`][ogx::composite_type] macro can consume.
*/
...
*/
#[og_extern]
fn create_dog(name: String, scritches: i32) -> ogx::composite_type!(DOG_COMPOSITE_TYPE) {
...
#[og_extern]
fn scritch_dog(
    mut dog: ogx::composite_type!(DOG_COMPOSITE_TYPE),
) -> ogx::composite_type!(DOG_COMPOSITE_TYPE) {
...
the function.
*/
#[og_extern(requires = ["create_cat_and_dog_friendship"])]
fn make_friendship(
    dog: ogx::composite_type!(DOG_COMPOSITE_TYPE),
    cat: ogx::composite_type!(CAT_COMPOSITE_TYPE),
) -> ogx::composite_type!(CAT_AND_DOG_FRIENDSHIP_COMPOSITE_TYPE) {
    let mut friendship =
...
impl Aggregate for SumScritches {
    ...
    type Args = ogx::name!(value, ogx::composite_type!("Dog"));
...
impl Aggregate for ScritchCollector {
    type State = Option<ogx::composite_type!("Dog")>;
    type Args = i32;
...
fn add_scritches_to_dog(
    mut left: ogx::composite_type!("Dog"),
    right: i32,
) -> ogx::composite_type!("Dog") {
