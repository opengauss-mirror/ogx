...
*/
use ogx_utils::sql_entity_graph::{OgHash, OgOrd};

use proc_macro2::Ident;
use quote::{quote, ToTokens};
use syn::DeriveInput;

pub(crate) fn impl_opengauss_eq(ast: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
...
pub(crate) fn impl_opengauss_ord(ast: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    let mut stream = proc_macro2::TokenStream::new();
    ...
    let sql_graph_entity_item = OgOrd::from_derive_input(ast)?;
    sql_graph_entity_item.to_tokens(&mut stream);

    Ok(stream)
}

pub(crate) fn impl_opengauss_hash(ast: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    let mut stream = proc_macro2::TokenStream::new();

    stream.extend(hash(&ast.ident));

    let sql_graph_entity_item = OgHash::from_derive_input(ast)?;
    sql_graph_entity_item.to_tokens(&mut stream);
...
pub fn hash(type_name: &Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        fn #pg_name(value: #type_name) -> i32 {
            ::ogx::misc::ogx_seahash(&value) as i32
        }
    }
}
