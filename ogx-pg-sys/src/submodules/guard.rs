...
impl OgxPanic {
    pub fn new(message: &'static str, filename: &'static str, lineno: u32, colno: u32) -> Self {
        OgxPanic { message, filename, lineno, colno }
    }
}
...
pub fn register_og_guard_panic_hook() {
...
/// behind the `#[og_guard]` and `#[og_extern]` macros.  Which means the function you'd like to guard
...
fn downcast_err(mut e: Box<dyn Any + Send>) -> Result<String, JumpContext> {
    if let Some(cxt) = e.downcast_ref::<JumpContext>() {
        ...
        Ok(mem::take(s))
    } else if let Some(s) = e.downcast_ref::<OgxPanic>() {
        let OgxPanic { message, filename, lineno, colno } = s;
