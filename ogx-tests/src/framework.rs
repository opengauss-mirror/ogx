...
use ogx::prelude::*;
use ogx_pg_config::{createdb, get_target_dir, PgConfig, Ogx};
...
fn get_pg_config() -> eyre::Result<PgConfig> {
    let ogx = Ogx::from_config().wrap_err("Unable to get OGX from config")?;

    let pg_version = pg_sys::get_pg_major_version_num();

    let pg_config = ogx
        .get(&format!("og{}", pg_version))
        .wrap_err_with(|| {
...
fn install_extension() -> eyre::Result<()> {
    ...
    let profile = std::env::var("OGX_BUILD_PROFILE").unwrap_or("debug".into());
    let no_schema = std::env::var("OGX_NO_SCHEMA").unwrap_or("false".into()) == "true";
    let mut features = std::env::var("OGX_FEATURES").unwrap_or("".to_string());
    if !features.contains("og_test") {
        features += " og_test";
    }
    let no_default_features =
        std::env::var("OGX_NO_DEFAULT_FEATURES").unwrap_or("false".to_string()) == "true";
    let all_features = std::env::var("OGX_ALL_FEATURES").unwrap_or("false".to_string()) == "true";

    let pg_version = format!("pg{}", pg_sys::get_pg_major_version_string());
    let ogx = Ogx::from_config()?;
    let pg_config = ogx.get(&pg_version)?;
    ...
    if let Ok(manifest_path) = std::env::var("OGX_MANIFEST_PATH") {
...
fn modify_postgresql_conf(pgdata: PathBuf, postgresql_conf: Vec<&'static str>) -> eyre::Result<()> {
    ...
    postgresql_conf_file
        .write_all(
            format!("unix_socket_directory = '{}'", Ogx::home().unwrap().display()).as_bytes(),
        )
        .wrap_err("couldn't append `unix_socket_directory` setting to postgresql.conf")?;
    Ok(())
}
...
fn dropdb() -> eyre::Result<()> {
    let pg_config = get_pg_config()?;
    let output = Command::new(pg_config.gsql_path().expect("unable to determine dropdb path"))
...
fn get_pgdata_path() -> eyre::Result<PathBuf> {
    ...
    target_dir.push(&format!("ogx-test-data-{}", pg_sys::get_pg_major_version_num()));
    Ok(target_dir)
}
