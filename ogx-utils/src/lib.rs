...
pub mod __reexports {
    pub use eyre;
    // For `#[no_std]` based `ogx` extensions we use `HashSet` for type mappings.
    pub mod std {
        pub mod collections {
                ...
                match archetype.ident.to_string().as_str() {
                    "name" => {
                        if let Ok(out) = mac.parse_body::<NameMacro>() {
                            ...
                            // which we end up using as a openGauss column name, to be nearly any
...
/// Roughly `ogx::pg_sys::NAMEDATALEN`
///
/// Technically it **should** be that exactly, however this is `ogx-utils` and a this data is used at macro time.
const OPENGAUSS_IDENTIFIER_MAX_LEN: usize = 64;

/// Validate that a given ident is acceptable to openGauss
///
/// openGauss places some restrictions on identifiers for things like functions.
///
/// Namely:
///
...
pub fn ident_is_acceptable_to_opengauss(ident: &syn::Ident) -> Result<(), syn::Error> {
    let ident_string = ident.to_string();
    if ident_string.len() >= OPENGAUSS_IDENTIFIER_MAX_LEN {
        return Err(syn::Error::new(
            ident.span(),
            &format!(
                "Identifier `{}` was {} characters long, openGauss will truncate identifiers with less than {OPENGAUSS_IDENTIFIER_MAX_LEN} characters, opt for an identifier which openGauss won't truncate",
                ident,
                ident_string.len(),
            )
        ));
