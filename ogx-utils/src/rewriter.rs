...
pub struct OgGuardRewriter();

impl OgGuardRewriter {
    pub fn new() -> Self {
        OgGuardRewriter()
    }
    ...
    pub fn item_fn_without_rewrite(
        ...
        let arg_list = OgGuardRewriter::build_arg_list(&sig, false)?;
        let func_name = OgGuardRewriter::build_func_name(&func.sig);

        let prolog = if input_func_name == "__ogx_private_shmem_hook" {
    ...
    pub fn foreign_item_fn(&self, func: &ForeignItemFn) -> eyre::Result<proc_macro2::TokenStream> {
        let func_name = OgGuardRewriter::build_func_name(&func.sig);
        let arg_list = OgGuardRewriter::rename_arg_list(&func.sig)?;
        let arg_list_with_types = OgGuardRewriter::rename_arg_list_with_types(&func.sig)?;
        let return_type = OgGuardRewriter::get_return_type(&func.sig);

        Ok(quote! {
            pub unsafe fn #func_name ( #arg_list_with_types ) #return_type {
                crate::submodules::setjmp::og_guard_ffi_boundary(move || {
                    extern "C" {
            ...
            match arg {
                FnArg::Typed(ty) => {
                    if let Pat::Ident(ident) = ty.pat.deref() {
                        ...
                        eyre::bail!(
                            "Unknown argument pattern in `#[og_guard]` function: `{:?}`",
                            ty.pat,
                        );
                    }
                }
                FnArg::Receiver(_) => panic!(
                    "#[og_guard] doesn't support external functions with 'self' as the argument"
                ),
            }
        }
