...
*/
#[derive(Debug, Clone)]
pub struct OgAggregate {
    item_impl: ItemImpl,
    name: Expr,
    og_externs: Vec<ItemFn>,
...
impl OgAggregate {
    pub fn new(mut item_impl: ItemImpl) -> Result<Self, syn::Error> {
        ...
        crate::ident_is_acceptable_to_opengauss(&snake_case_target_ident)?;

        let mut og_externs = Vec::default();
        ...
        if let Some((_, ref path, _)) = item_impl.trait_ {
            // TODO: Consider checking the path if there is more than one segment to make sure it's ogx.
        ...
        let fn_state_name = if let Some(found) = fn_state {
            ...
            let og_extern_attr = og_extern_attr(found);

            og_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #og_extern_attr
                fn #fn_name(this: #type_state_without_self, #(#args_with_names),*, fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_state_without_self {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::state(this, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            fn_name
        ...
        let fn_combine_name = if let Some(found) = fn_combine {
            ...
            og_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #og_extern_attr
                fn #fn_name(this: #type_state_without_self, v: #type_state_without_self, fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_state_without_self {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::combine(this, v, fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn combine(current: #type_state_without_self, _other: #type_state_without_self, _fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_state_without_self {
        ...
        let fn_finalize_name = if let Some(found) = fn_finalize {
            ...
            if direct_args_with_names.len() > 0 {
                og_externs.push(parse_quote! {
                    #[allow(non_snake_case, clippy::too_many_arguments)]
                    #og_extern_attr
                    fn #fn_name(this: #type_state_without_self, #(#direct_args_with_names),*, fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_finalize {
                        <#target_path as ogx::Aggregate>::in_memory_context(
                            fcinfo,
                            move |_context| <#target_path as ogx::Aggregate>::finalize(this, (#(#direct_arg_names),*), fcinfo)
                        )
                    }
                });
            } else {
                og_externs.push(parse_quote! {
                    #[allow(non_snake_case, clippy::too_many_arguments)]
                    #og_extern_attr
                    fn #fn_name(this: #type_state_without_self, fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_finalize {
                        <#target_path as ::Aggregate>::in_memory_context(
                            fcinfo,
                            move |_context| <#target_path as ogx::Aggregate>::finalize(this, (), fcinfo)
                        )
                    }
                });
            };
            ...
            item_impl.items.push(parse_quote! {
                fn finalize(current: Self::State, direct_args: Self::OrderedSetArgs, _fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_finalize {
        ...
        let fn_serial_name = if let Some(found) = fn_serial {
            ...
            og_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #og_extern_attr
                fn #fn_name(this: #type_state_without_self, fcinfo: ogx::pg_sys::FunctionCallInfo) -> Vec<u8> {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::serial(this, fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn serial(current: #type_state_without_self, _fcinfo: ogx::pg_sys::FunctionCallInfo) -> Vec<u8> {
        ...
        let fn_deserial_name = if let Some(found) = fn_deserial {
            ...
            og_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #og_extern_attr
                fn #fn_name(this: #type_state_without_self, buf: Vec<u8>, internal: ogx::OgBox<#type_state_without_self>, fcinfo: ogx::pg_sys::FunctionCallInfo) -> ogx::OgBox<#type_state_without_self> {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::deserial(this, buf, internal, fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn deserial(current: #type_state_without_self, _buf: Vec<u8>, _internal: ogx::OgBox<#type_state_without_self>, _fcinfo: ogx::pg_sys::FunctionCallInfo) -> ogx::OgBox<#type_state_without_self> {
        ...
        let fn_moving_state_name = if let Some(found) = fn_moving_state {
            ...
            og_externs.push(parse_quote! {
                ...
                fn #fn_name(
                    ...
                    #(#args_with_names),*,
                    fcinfo: ogx::pg_sys::FunctionCallInfo,
                ) -> #type_moving_state {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::moving_state(mstate, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn moving_state(
                    _mstate: <#target_path as ogx::Aggregate>::MovingState,
                    _v: Self::Args,
                    _fcinfo: ogx::pg_sys::FunctionCallInfo,
                ) -> <#target_path as ogx::Aggregate>::MovingState {
        ...
        let fn_moving_state_inverse_name = if let Some(found) = fn_moving_state_inverse {
            ...
            og_externs.push(parse_quote! {
                ...
                fn #fn_name(
                    ...
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::moving_state_inverse(mstate, (#(#arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
        ...
        let fn_moving_finalize_name = if let Some(found) = fn_moving_finalize {
            ...
            og_externs.push(parse_quote! {
                #[allow(non_snake_case, clippy::too_many_arguments)]
                #og_extern_attr
                fn #fn_name(mstate: #type_moving_state, #(#direct_args_with_names),* #maybe_comma fcinfo: ogx::pg_sys::FunctionCallInfo) -> #type_finalize {
                    <#target_path as ogx::Aggregate>::in_memory_context(
                        fcinfo,
                        move |_context| <#target_path as ogx::Aggregate>::moving_finalize(mstate, (#(#direct_arg_names),*), fcinfo)
                    )
                }
            });
            Some(fn_name)
            ...
            item_impl.items.push(parse_quote! {
                fn moving_finalize(_mstate: Self::MovingState, direct_args: Self::OrderedSetArgs, _fcinfo: ogx::pg_sys::FunctionCallInfo) -> Self::Finalize {
    ...
    fn entity_tokens(&self) -> ItemFn {
        let target_path = get_target_path(&self.item_impl)
            .expect("Expected constructed OgAggregate to have target path.");
        let target_ident = get_target_ident(&target_path)
            .expect("Expected constructed OgAggregate to have target ident.");
        let snake_case_target_ident =
            Ident::new(&target_ident.to_string().to_case(Case::Snake), target_ident.span());
        let sql_graph_entity_fn_name = syn::Ident::new(
            &format!("__ogx_internals_aggregate_{}", snake_case_target_ident),
            target_ident.span(),
        );
        ...
        let entity_item_fn: ItemFn = parse_quote! {
            ...
            pub extern "Rust" fn #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                let submission = ::ogx::utils::sql_entity_graph::OgAggregateEntity {
                ...
                ::ogx::utils::sql_entity_graph::SqlGraphEntity::Aggregate(submission)
            }
        };
        entity_item_fn
    }
...
fn og_extern_attr(item: &ImplItemMethod) -> syn::Attribute {
    let mut found = None;
    for attr in item.attrs.iter() {
        match attr.path.segments.last() {
            Some(segment) if segment.ident.to_string() == "ogx" => {
...
fn get_ogx_attr_macro(attr_name: impl AsRef<str>, ty: &syn::Type) -> Option<TokenStream2> {
    match &ty {
        syn::Type::Macro(ty_macro) => {
            let mut found_ogx = false;
            ...
            for (idx, segment) in ty_macro.mac.path.segments.iter().enumerate() {
                match segment.ident.to_string().as_str() {
                    "ogx" if idx == 0 => found_ogx = true,
                    attr if attr == attr_name.as_ref() => found_attr = true,
                    _ => (),
                }
            }
            if (ty_macro.mac.path.segments.len() == 1 && found_attr) || (found_ogx && found_attr) {
...
mod tests {
    ...
    fn agg_required_only() -> Result<()> {
        ...
        let agg = OgAggregate::new(tokens);
        assert!(agg.is_ok());
        // It should create 1 extern, the state.
        let agg = agg.unwrap();
        assert_eq!(agg.og_externs.len(), 1);
        // That extern should be named specifically:
        let extern_fn = &agg.og_externs[0];
    ...
    fn agg_all_options() -> Result<()> {
        let tokens: ItemImpl = parse_quote! {
            ...
            impl Aggregate for DemoAgg {
                ...
                fn deserial(current: Self::State, _buf: Vec<u8>, _internal: OgBox<Self>) -> OgBox<Self> {
                    todo!()
                }
        ...
        let agg = agg.unwrap();
        assert_eq!(agg.og_externs.len(), 8);
        // That extern should be named specifically:
        let extern_fn = &agg.og_externs[0];
        assert_eq!(extern_fn.sig.ident.to_string(), "demo_agg_state");
        // It should be possible to generate entity tokens.
