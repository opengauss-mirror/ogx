...
`ogx::extension_sql!()` related macro expansion for Rust to SQL translation
...
/// use ogx_utils::sql_entity_graph::ExtensionSqlFile;
///
/// # fn main() -> eyre::Result<()> {
...
impl ToTokens for ExtensionSqlFile {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let sql_graph_entity_fn_name =
            syn::Ident::new(&format!("__ogx_internals_sql_{}", name.clone()), Span::call_site());
        let inv = quote! {
            #[no_mangle]
            #[doc(hidden)]
            pub extern "Rust" fn  #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                ...
                use alloc::vec::Vec;
                use alloc::vec;
                let submission = ::ogx::utils::sql_entity_graph::ExtensionSqlEntity {
                ...
                ::ogx::utils::sql_entity_graph::SqlGraphEntity::CustomSql(submission)
            }
        };
...
/// It should be used with [`syn::parse::Parse`] functions.
///
/// Using [`quote::ToTokens`] will output the declaration for a `ogx::utils::sql_entity_graph::ExtensionSqlEntity`.
///
/// ```rust
/// use syn::{Macro, parse::Parse, parse_quote, parse};
/// use quote::{quote, ToTokens};
/// use ogx_utils::sql_entity_graph::ExtensionSql;
///
/// # fn main() -> eyre::Result<()> {
/// let parsed: Macro = parse_quote! {
...
impl ToTokens for ExtensionSql {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let sql_graph_entity_fn_name =
            syn::Ident::new(&format!("__ogx_internals_sql_{}", name.value()), Span::call_site());
        let inv = quote! {
            #[no_mangle]
            pub extern "Rust" fn  #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                extern crate alloc;
...
impl ToTokens for SqlDeclared {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let inv = quote! {
            ::ogx::utils::sql_entity_graph::SqlDeclaredEntity::build(#variant, #identifier).unwrap()
        };
        tokens.append_all(inv);
    }
}
