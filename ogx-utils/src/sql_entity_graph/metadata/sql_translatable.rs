...
pub enum SqlMapping {
    /// Explicit mappings provided by OGX
    As(String),
    Composite {
        array_brackets: bool,
    },
...
By implementing this, you assert you are not lying to either openGauss or Rust in doing so.
This trait asserts a safe translation exists between values of this type from Rust to SQL,
or from SQL into Rust. If you are mistaken about how this works, either the openGauss C API
or the Rust handling in OGX may emit undefined behavior.

It cannot be made private or sealed due to details of the structure of the OGX framework.
