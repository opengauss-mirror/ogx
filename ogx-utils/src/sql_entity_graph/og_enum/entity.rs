...
`#[derive(OgEnum)]` related entities for Rust to SQL translation
...
/// The output of a [`OgEnum`](crate::sql_entity_graph::og_enum::OgEnum) from `quote::ToTokens::to_tokens`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OgEnumEntity {
    pub name: &'static str,
    pub file: &'static str,
    pub line: u32,
...
impl Hash for OgEnumEntity {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.full_path.hash(state);
    }
}

impl Ord for OgEnumEntity {
    fn cmp(&self, other: &Self) -> Ordering {
        self.file.cmp(other.file).then_with(|| self.file.cmp(other.file))
    }
}

impl PartialOrd for OgEnumEntity {
...
impl From<OgEnumEntity> for SqlGraphEntity {
    fn from(val: OgEnumEntity) -> Self {
        SqlGraphEntity::Enum(val)
    }
}

impl SqlGraphIdentifier for OgEnumEntity {
