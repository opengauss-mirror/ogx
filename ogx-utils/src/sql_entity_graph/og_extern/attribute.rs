...
`#[og_extern]` related attributes for Rust to SQL translation
...
impl Attribute {
    pub(crate) fn to_sql_entity_graph_tokens(&self) -> TokenStream2 {
        match self {
            Attribute::Immutable => quote! { ::ogx::utils::ExternArgs::Immutable },
            Attribute::Strict => quote! { ::ogx::utils::ExternArgs::Strict },
            Attribute::Stable => quote! { ::ogx::utils::ExternArgs::Stable },
            Attribute::Volatile => quote! { ::ogx::utils::ExternArgs::Volatile },
            Attribute::Raw => quote! { ::ogx::utils::ExternArgs::Raw },
            Attribute::NoGuard => quote! { ::ogx::utils::ExternArgs::NoGuard },
            Attribute::CreateOrReplace => quote! { ::ogx::utils::ExternArgs::CreateOrReplace },
            Attribute::ParallelSafe => {
                quote! { ::ogx::utils::ExternArgs::ParallelSafe }
            }
            Attribute::ParallelUnsafe => {
                quote! { ::ogx::utils::ExternArgs::ParallelUnsafe }
            }
            Attribute::ParallelRestricted => {
                quote! { ::ogx::utils::ExternArgs::ParallelRestricted }
            }
            Attribute::Error(s) => {
                quote! { ::ogx::utils::ExternArgs::Error(String::from(#s)) }
            }
            Attribute::Schema(s) => {
                quote! { ::ogx::utils::ExternArgs::Schema(String::from(#s)) }
            }
            Attribute::Name(s) => {
                quote! { ::ogx::utils::ExternArgs::Name(String::from(#s)) }
            }
            Attribute::Cost(s) => {
                quote! { ::ogx::utils::ExternArgs::Cost(format!("{}", #s)) }
            }
            Attribute::Requires(items) => {
                let items_iter = items.iter().map(|x| x.to_token_stream()).collect::<Vec<_>>();
                quote! { ::::utils::ExternArgs::Requires(vec![#(#items_iter),*],) }
            }
...
impl Parse for Attribute {
    fn parse(input: ParseStream) -> Result<Self, syn::Error> {
        ...
        let found = match ident.to_string().as_str() {
            ...
            "sql" => {
                use crate::sql_entity_graph::ogx_attribute::ArgValue;
                use syn::Lit;
