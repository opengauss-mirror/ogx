...
`#[og_extern]` related entities for Rust to SQL translation
...
pub use argument::OgExternArgumentEntity;
pub use operator::OgOperatorEntity;
pub use returning::{OgExternReturnEntity, OgExternReturnEntityIteratedItem};
...
/// The output of a [`OgExtern`](crate::sql_entity_graph::og_extern::OgExtern) from `quote::ToTokens::to_tokens`.
#[derive(Debug, Clone)]
pub struct OgExternEntity {
    pub name: &'static str,
    pub unaliased_name: &'static str,
    pub module_path: &'static str,
    pub full_path: &'static str,
    pub metadata: crate::sql_entity_graph::metadata::FunctionMetadataEntity,
    pub fn_args: Vec<OgExternArgumentEntity>,
    pub fn_return: OgExternReturnEntity,
    pub schema: Option<&'static str>,
    pub file: &'static str,
    pub line: u32,
    pub extern_attrs: Vec<ExternArgs>,
    pub search_path: Option<Vec<&'static str>>,
    pub operator: Option<OgOperatorEntity>,
    pub to_sql_config: ToSqlConfigEntity,
}

impl std::hash::Hash for OgExternEntity {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.full_path.hash(state);
    }
}

impl PartialEq for OgExternEntity {
    fn eq(&self, other: &Self) -> bool {
        self.full_path.eq(other.full_path)
    }
}

impl Eq for OgExternEntity {}

impl Ord for OgExternEntity {
    fn cmp(&self, other: &Self) -> Ordering {
        self.full_path.cmp(&other.full_path)
    }
}

impl PartialOrd for OgExternEntity {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl From<OgExternEntity> for SqlGraphEntity {
    fn from(val: OgExternEntity) -> Self {
        SqlGraphEntity::Function(val)
    }
}

impl SqlGraphIdentifier for OgExternEntity {
...
impl ToSql for OgExternEntity {
    ...
    fn to_sql(&self, context: &OgxSql) -> eyre::Result<String> {
        ...
        if strict_upgrade {
            ...
            // But we can only do that if the user hasn't used `Option<T>` or `ogx::Internal`
        ...
        let fn_sql = format!(
            ...
            returns = match &self.fn_return {
                OgExternReturnEntity::None => String::from("RETURNS void"),
                OgExternReturnEntity::Type { ty } => {
                    let graph_index = context
                        .graph
                        .neighbors_undirected(self_index)
                        .find(|neighbor| match &context.graph[*neighbor] {
                ...
                OgExternReturnEntity::SetOf { ty, optional: _ } => {
                    let graph_index = context
                        .graph
                        .neighbors_undirected(self_index)
                        .find(|neighbor| match &context.graph[*neighbor] {
                ...
                OgExternReturnEntity::Iterated {
                    ...
                    for (idx, returning::OgExternReturnEntityIteratedItem { ty, name: col_name }) in
                ...
                OgExternReturnEntity::Trigger => String::from("RETURNS trigger"),
            },
                ...
                Ok(SqlMapping::Composite { array_brackets }) => {
                    if array_brackets {
                        let composite_type = self.fn_args[0].used_ty.composite_type
                            .ok_or(eyre!("Found a composite type but macro expansion time did not reveal a name, use `ogx::composite_type!()`"))?;
                        format!("{composite_type}[]")
                    } else {
                        self.fn_args[0].used_ty.composite_type
                            .ok_or(eyre!("Found a composite type but macro expansion time did not reveal a name, use `ogx::composite_type!()`"))?.to_string()
                    }
                }
