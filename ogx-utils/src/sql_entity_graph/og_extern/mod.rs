...
`#[og_extern]` related macro expansion for Rust to SQL translation
...
pub use argument::OgExternArgument;
pub use operator::OgOperator;
pub use returning::NameMacro;

use crate::sql_entity_graph::ToSqlConfig;
use crate::staticize_lifetimes;
use attribute::Attribute;
use operator::{OgxOperatorAttributeWithIdent, OgxOperatorOpName};
...
/// A parsed `#[og_extern]` item.
///
/// It should be used with [`syn::parse::Parse`] functions.
///
/// Using [`quote::ToTokens`] will output the declaration for a [`OgExternEntity`][crate::sql_entity_graph::OgExternEntity].
///
/// ```rust
/// use syn::{Macro, parse::Parse, parse_quote, parse};
/// use quote::{quote, ToTokens};
/// use ogx_utils::sql_entity_graph::OgExtern;
///
/// # fn main() -> eyre::Result<()> {
/// let parsed: OgExtern = parse_quote! {
...
impl OgExtern {
            ...
            if let Some(meta) = meta {
                if meta.path().is_ident("doc") {
                    ...
                    if let syn::Lit::Str(ref inner) = content.lit {
                        ...
                        if !in_commented_sql_block && inner.value().trim() == "```ogxsql" {
    ...
    fn operator(&self) -> Option<OgOperator> {
        let mut skel = Option::<OgOperator>::default();
        for attr in &self.func.attrs {
            let last_segment = attr.path.segments.last().unwrap();
            match last_segment.ident.to_string().as_str() {
                "opname" => {
                    let attr: OgxOperatorOpName = syn::parse2(attr.tokens.clone())
                        .expect(&format!("Unable to parse {:?}", &attr.tokens));
                    skel.get_or_insert_with(Default::default).opname.get_or_insert(attr);
                }
                "commutator" => {
                    let attr: OgxOperatorAttributeWithIdent = syn::parse2(attr.tokens.clone())
                        .expect(&format!("Unable to parse {:?}", &attr.tokens));
    ...
    fn inputs(&self) -> eyre::Result<Vec<OgExternArgument>> {
        let mut args = Vec::default();
        for input in &self.func.sig.inputs {
            let arg = OgExternArgument::build(input.clone())
    ...
    fn entity_tokens(&self) -> TokenStream2 {
        ...
        let sql_graph_entity_fn_name =
            syn::Ident::new(&format!("__ogx_internals_fn_{}", ident), Span::call_site());
        quote_spanned! { self.func.sig.span() =>
            ...
            pub extern "Rust" fn  #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                ...
                let submission = ::ogx::utils::sql_entity_graph::OgExternEntity {
                    name: #name,
                    unaliased_name: stringify!(#ident),
                    module_path: core::module_path!(),
                    full_path: concat!(core::module_path!(), "::", stringify!(#ident)),
                    metadata: ogx::utils::sql_entity_graph::metadata::FunctionMetadata::entity(&metadata),
                ...
                ::ogx::utils::sql_entity_graph::SqlGraphEntity::Function(submission)
            }
        }
    }
    ...
    pub fn wrapper_func(&self) -> TokenStream2 {
        ...
        let arg_fetches = args.iter().enumerate().map(|(idx, arg)| {
            ...
            if arg.used_ty.resolved_ty.to_token_stream().to_string() == quote!(ogx::pg_sys::FunctionCallInfo).to_token_stream().to_string()
            || arg.used_ty.resolved_ty.to_token_stream().to_string() == quote!(pg_sys::FunctionCallInfo).to_token_stream().to_string() {
                ...
                quote_spanned! {pat.span()=>
                    debug_assert!(ogx::pg_getarg::<()>(#fcinfo_ident, #idx).is_none(), "A `()` argument should always recieve `NULL`");
                    let #pat = ();
                }
            } else {
                match (is_raw, &arg.used_ty.optional) {
                    (true, None) | (true, Some(_)) => quote_spanned! { pat.span() =>
                        let #pat = ogx::pg_getarg_datum_raw(#fcinfo_ident, #idx) as #resolved_ty;
                    },
                    (false, None) => quote_spanned! { pat.span() =>
                        let #pat = ogx::pg_getarg::<#resolved_ty>(#fcinfo_ident, #idx).unwrap_or_else(|| panic!("{} is null", stringify!{#pat}));
                    },
                    (false, Some(inner)) => quote_spanned! { pat.span() =>
                        let #pat = ogx::pg_getarg::<#inner>(#fcinfo_ident, #idx);
                    },
                }
            }
        });
        ...
        match self.returns().unwrap() {
            Returning::None => quote_spanned! { self.func.sig.span() =>
                    ...
                    #[doc(hidden)]
                    #[og_guard]
                    pub unsafe extern "C" fn #func_name_wrapper #func_generics(#fcinfo_ident: ::ogx::pg_sys::FunctionCallInfo) {
                        #(
                            #arg_fetches
                        )*
            ...
            Returning::Type(retval_ty) => {
                ...
                let retval_transform = if retval_ty.resolved_ty == syn::parse_quote!(()) {
                    ...
                    || retval_ty.resolved_ty == syn::parse_quote!(ogx::pg_sys::Datum)
                {
                    quote_spanned! { self.func.sig.output.span() =>
                       #result_ident
                    }
                    ...
                    quote_spanned! { self.func.sig.output.span() =>
                        match #result_ident {
                            Some(result) => {
                                ogx::datum::IntoDatum::into_datum(result).unwrap_or_else(|| panic!("returned Option<T> was NULL"))
                            },
                            None => ogx::pg_return_null(#fcinfo_ident)
                        }
                    }
                } else {
                    quote_spanned! { self.func.sig.output.span() =>
                        ogx::datum::IntoDatum::into_datum(#result_ident).unwrap_or_else(|| panic!("returned Datum was NULL"))
                    }
                };
                ...
                quote_spanned! { self.func.sig.span() =>
                    ...
                    #[doc(hidden)]
                    #[og_guard]
                    pub unsafe extern "C" fn #func_name_wrapper #func_generics(#fcinfo_ident: ::ogx::pg_sys::FunctionCallInfo) -> ::ogx::pg_sys::Datum {
                        #(
                            #arg_fetches
                        )*
            ...
            Returning::SetOf {
                ...
                quote_spanned! { self.func.sig.span() =>
                    ...
                    pub unsafe extern "C" fn #func_name_wrapper #func_generics(#fcinfo_ident: ::ogx::pg_sys::FunctionCallInfo) -> ::ogx::pg_sys::Datum {
                        struct IteratorHolder<'__ogx_internal_lifetime, T: std::panic::UnwindSafe + std::panic::RefUnwindSafe> {
                            iter: *mut SetOfIterator<'__ogx_internal_lifetime, T>,
                        }

                        let mut funcctx: ::ogx::OgBox<pg_sys::FuncCallContext>;
                        let mut iterator_holder: ::ogx::OgBox<IteratorHolder<#retval_ty_resolved>>;

                        unsafe {
                            if ::ogx::srf_is_first_call(#fcinfo_ident) {
                                funcctx = ogx::srf_first_call_init(#fcinfo_ident);
                                funcctx.user_fctx = ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).palloc_struct::<IteratorHolder<#retval_ty_resolved>>() as *mut ::core::ffi::c_void;
                                iterator_holder = ogx::OgBox::from_pg(funcctx.user_fctx as *mut IteratorHolder<#retval_ty_resolved>);
                                ...
                                let #result_ident = match ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).switch_to(|_| {
                                    #( #arg_fetches )*
                                    #result_handler
                                }) {
                                    Some(result) => result,
                                    None => {
                                        ogx::srf_return_done(#fcinfo_ident, &mut funcctx);
                                        return ogx::pg_return_null(#fcinfo_ident)
                                    }
                                };

                                iterator_holder.iter = ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).leak_trivial_alloc(result);
                            }

                            funcctx = ogx::srf_per_call_setup(#fcinfo_ident);
                            iterator_holder = ogx::OgBox::from_pg(funcctx.user_fctx as *mut IteratorHolder<#retval_ty_resolved>);
                        }
                        ...
                        match iter.next() {
                            Some(result) => {
                                ...
                                unsafe { ogx::srf_return_next(#fcinfo_ident, &mut funcctx) };
                                match ogx::datum::IntoDatum::into_datum(result) {
                                    Some(datum) => datum,
                                    None => ogx::pg_return_null(#fcinfo_ident),
                                }
                            },
                            None => {
                                ...
                                unsafe { ogx::srf_return_done(#fcinfo_ident, &mut funcctx) };
                                ogx::pg_return_null(#fcinfo_ident)
                            },
                        }
                    }
                }
            ...
            Returning::Iterated {
                ...
                let create_heap_tuple = quote! {
                    let mut datums: [::ogx::pg_sys::Datum; #retval_tuple_len] = [::ogx::pg_sys::Datum::from(0); #retval_tuple_len];
                    let mut nulls: [bool; #retval_tuple_len] = [false; #retval_tuple_len];

                    #(
                        let datum = ogx::datum::IntoDatum::into_datum(result.#retval_tuple_indexes);
                        match datum {
                    ...
                    let heap_tuple = unsafe { ogx::pg_sys::heap_form_tuple(#funcctx_ident.tuple_desc, datums.as_mut_ptr(), nulls.as_mut_ptr()) };
                };
                ...
                quote_spanned! { self.func.sig.span() =>
                    ...
                    pub unsafe extern "C" fn #func_name_wrapper #func_generics(#fcinfo_ident: ::ogx::pg_sys::FunctionCallInfo) -> ::ogx::pg_sys::Datum {
                        struct IteratorHolder<'__ogx_internal_lifetime, T: std::panic::UnwindSafe + std::panic::RefUnwindSafe> {
                            iter: *mut ::ogx::iter::TableIterator<'__ogx_internal_lifetime, T>,
                        }

                        let mut funcctx: ogx::OgBox<pg_sys::FuncCallContext>;
                        let mut iterator_holder: ogx::OgBox<IteratorHolder<#retval_tys_tuple>>;

                        unsafe {
                            if ::ogx::srf_is_first_call(#fcinfo_ident) {
                                funcctx = ::ogx::srf_first_call_init(#fcinfo_ident);
                                funcctx.user_fctx = ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).palloc_struct::<IteratorHolder<#retval_tys_tuple>>() as *mut ::core::ffi::c_void;
                                funcctx.tuple_desc = ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).switch_to(|_| {
                                    let mut tupdesc: *mut ogx::pg_sys::TupleDescData = std::ptr::null_mut();

                                    /* Build a tuple descriptor for our result type */
                                    if ogx::pg_sys::get_call_result_type(#fcinfo_ident, std::ptr::null_mut(), &mut tupdesc) != ogx::pg_sys::TypeFuncClass_TYPEFUNC_COMPOSITE {
                                        ogx::error!("return type must be a row type");
                                    }

                                    ogx::pg_sys::BlessTupleDesc(tupdesc)
                                });
                                iterator_holder = ogx::OgBox::from_pg(funcctx.user_fctx as *mut IteratorHolder<#retval_tys_tuple>);
                                ...
                                iterator_holder.iter = ogx::OgMemoryContexts::For(funcctx.multi_call_memory_ctx).leak_and_drop_on_delete(result);
                            }
                        ...
                        match iter.next() {
                            Some(result) => {
                                ...
                                let datum = ogx::heap_tuple_get_datum(heap_tuple);
                                // SAFETY: what is an srf if it does not return?
                                unsafe { ogx::srf_return_next(#fcinfo_ident, &mut funcctx) };
                                ogx::pg_sys::Datum::from(datum)
                            },
                            None => {
                                // leak the iterator here too, even tho we're done, b/c our MemoryContextCallback
                                // function is going to properly drop it for us
