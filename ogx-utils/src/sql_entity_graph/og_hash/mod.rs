...
`#[derive(OgHash)]` related macro expansion for Rust to SQL translation
...
/// A parsed `#[derive(OgHash)]` item.
///
/// It should be used with [`syn::parse::Parse`] functions.
///
/// Using [`quote::ToTokens`] will output the declaration for a `ogx::datum::sql_entity_graph::InventoryOgHash`.
///
/// On structs:
///
/// ```rust
...
/// use ogx_utils::sql_entity_graph::OgHash;
///
/// # fn main() -> eyre::Result<()> {
/// let parsed: OgHash = parse_quote! {
///     #[derive(OgHash)]
///     struct Example<'a> {
///         demo: &'a str,
///     }
...
impl ToTokens for OgHash {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        let name = &self.name;
        let sql_graph_entity_fn_name =
            syn::Ident::new(&format!("__ogx_internals_hash_{}", self.name), Span::call_site());
        ...
        let inv = quote! {
            ...
            pub extern "Rust" fn  #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                ...
                use alloc::vec::Vec;
                use alloc::vec;
                let submission = ::ogx::utils::sql_entity_graph::OgHashEntity {
                    name: stringify!(#name),
                    file: file!(),
                    line: line!(),
                ...
                ::ogx::utils::sql_entity_graph::SqlGraphEntity::Hash(submission)
            }
        };
