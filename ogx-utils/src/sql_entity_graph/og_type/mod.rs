...
`#[derive(OgType)]` related macro expansion for Rust to SQL translation
...
/// A parsed `#[derive(OgType)]` item.
///
/// It should be used with [`syn::parse::Parse`] functions.
///
/// Using [`quote::ToTokens`] will output the declaration for a [`OgTypeEntity`][crate::sql_entity_graph::OgTypeEntity].
///
/// ```rust
/// use syn::{Macro, parse::Parse, parse_quote, parse};
/// use quote::{quote, ToTokens};
/// use ogx_utils::sql_entity_graph::OgType;
///
/// # fn main() -> eyre::Result<()> {
/// let parsed: OgType = parse_quote! {
///     #[derive(OgType)]
///     struct Example<'a> {
///         demo: &'a str,
///     }
...
impl ToTokens for OgType {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let sql_graph_entity_fn_name =
            syn::Ident::new(&format!("__ogx_internals_type_{}", self.name), Span::call_site());
        ...
        let inv = quote! {
            ...
            pub extern "Rust" fn  #sql_graph_entity_fn_name() -> ::ogx::utils::sql_entity_graph::SqlGraphEntity {
                ...
                <#name #static_ty_generics as ogx::datum::WithTypeIds>::register_with_refs(
                    &mut mappings,
                    stringify!(#name).to_string()
                );
                ogx::datum::WithSizedTypeIds::<#name #static_ty_generics>::register_sized_with_refs(
                    &mut mappings,
                    stringify!(#name).to_string()
                );
                ogx::datum::WithArrayTypeIds::<#name #static_ty_generics>::register_array_with_refs(
                    &mut mappings,
                    stringify!(#name).to_string()
                );
                ogx::datum::WithVarlenaTypeIds::<#name #static_ty_generics>::register_varlena_with_refs(
                    &mut mappings,
                    stringify!(#name).to_string()
                );
                let submission = ::ogx::utils::sql_entity_graph::OgTypeEntity {
                    name: stringify!(#name),
                    file: file!(),
                    line: line!(),
                ...
                ::ogx::utils::sql_entity_graph::SqlGraphEntity::Type(submission)
            }
        };
