...
`#[ogx]` attribute for Rust to SQL mapping support.
...
/// This struct is intented to represent the contents of the `#[ogx]` attribute when parsed.
///
/// The intended usage is to parse an `Attribute`, then use `attr.parse_args::<OgxAttribute>()?` to
...
pub struct OgxAttribute {
    pub args: Vec<OgxArg>,
}

impl Parse for OgxAttribute {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        let parser = Punctuated::<OgxArg, Token![,]>::parse_terminated;
