...
use crate::sql_entity_graph::aggregate::entity::OgAggregateEntity;
...
use crate::sql_entity_graph::og_extern::entity::OgExternEntity;
use crate::sql_entity_graph::og_trigger::entity::OgTriggerEntity;
use crate::sql_entity_graph::positioning_ref::PositioningRef;
use crate::sql_entity_graph::og_enum::entity::OgEnumEntity;
use crate::sql_entity_graph::og_hash::entity::OgHashEntity;
use crate::sql_entity_graph::og_ord::entity::OgOrdEntity;
use crate::sql_entity_graph::og_type::entity::OgTypeEntity;
...
use super::{OgExternReturnEntity, OgExternReturnEntityIteratedItem};
...
/// A generator for SQL.
///
/// Consumes a base mapping of types (typically `ogx::DEFAULT_TYPEID_SQL_MAPPING`), a
...
/// `animals::Dog`.
///
/// Typically, [`OgxSql`] types are constructed in a `ogx::pg_binary_magic!()` call in a binary
/// out of entities collected during a `ogx::pg_module_magic!()` call in a library.
#[derive(Debug, Clone)]
pub struct OgxSql {
    ...
    pub externs: HashMap<OgExternEntity, NodeIndex>,
    pub types: HashMap<OgTypeEntity, NodeIndex>,
    pub builtin_types: HashMap<String, NodeIndex>,
    pub enums: HashMap<OgEnumEntity, NodeIndex>,
    pub ords: HashMap<OgOrdEntity, NodeIndex>,
    pub hashes: HashMap<OgHashEntity, NodeIndex>,
    pub aggregates: HashMap<OgAggregateEntity, NodeIndex>,
    pub triggers: HashMap<OgTriggerEntity, NodeIndex>,
...
impl OgxSql {
    ...
    pub fn build(
        ...
        let mut externs: Vec<OgExternEntity> = Vec::default();
        let mut types: Vec<OgTypeEntity> = Vec::default();
        let mut enums: Vec<OgEnumEntity> = Vec::default();
        let mut ords: Vec<OgOrdEntity> = Vec::default();
        let mut hashes: Vec<OgHashEntity> = Vec::default();
        let mut aggregates: Vec<OgAggregateEntity> = Vec::default();
        let mut triggers: Vec<OgTriggerEntity> = Vec::default();
    ...
    pub fn get_module_pathname(&self) -> String {
        return if self.versioned_so {
            ...
            // Note: versioned so-name format must agree with cargo ogx
...
pub fn find_positioning_ref_target<'a>(
    ...
    types: &'a HashMap<OgTypeEntity, NodeIndex>,
    enums: &'a HashMap<OgEnumEntity, NodeIndex>,
    externs: &'a HashMap<OgExternEntity, NodeIndex>,
    schemas: &'a HashMap<SchemaEntity, NodeIndex>,
    extension_sqls: &'a HashMap<ExtensionSqlEntity, NodeIndex>,
    triggers: &'a HashMap<OgTriggerEntity, NodeIndex>,
) -> Option<&'a NodeIndex> {
...
fn connect_extension_sqls(
    ...
    types: &HashMap<OgTypeEntity, NodeIndex>,
    enums: &HashMap<OgEnumEntity, NodeIndex>,
    externs: &HashMap<OgExternEntity, NodeIndex>,
    triggers: &HashMap<OgTriggerEntity, NodeIndex>,
) -> eyre::Result<()> {
...
fn initialize_enums(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    enums: Vec<OgEnumEntity>,
) -> eyre::Result<HashMap<OgEnumEntity, NodeIndex>> {
...
fn initialize_types(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    types: Vec<OgTypeEntity>,
) -> eyre::Result<HashMap<OgTypeEntity, NodeIndex>> {
...
fn initialize_externs(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    externs: Vec<OgExternEntity>,
    mapped_types: &HashMap<OgTypeEntity, NodeIndex>,
    mapped_enums: &HashMap<OgEnumEntity, NodeIndex>,
) -> eyre::Result<(HashMap<OgExternEntity, NodeIndex>, HashMap<String, NodeIndex>)> {
    ...
    for item in externs {
        ...
        match &item.fn_return {
            OgExternReturnEntity::None | OgExternReturnEntity::Trigger => (),
            OgExternReturnEntity::Type { ty, .. } | OgExternReturnEntity::SetOf { ty, .. } => {
                let mut found = false;
                for (ty_item, &_ty_index) in mapped_types {
            ...
            OgExternReturnEntity::Iterated { tys: iterated_returns, optional: _ } => {
                for OgExternReturnEntityIteratedItem { ty: return_ty_entity, .. } in
                    iterated_returns
                {
                    let mut found = false;
                    for (ty_item, &_ty_index) in mapped_types {
...
fn connect_externs(
    graph: &mut StableGraph<SqlGraphEntity, SqlGraphRelationship>,
    externs: &HashMap<OgExternEntity, NodeIndex>,
    hashes: &HashMap<OgHashEntity, NodeIndex>,
    schemas: &HashMap<SchemaEntity, NodeIndex>,
    types: &HashMap<OgTypeEntity, NodeIndex>,
    enums: &HashMap<OgEnumEntity, NodeIndex>,
    ...
    for (item, &index) in externs {
        ...
        match &item.fn_return {
            ...
            OgExternReturnEntity::Iterated { tys: iterated_returns, optional: _ } => {
                for OgExternReturnEntityIteratedItem { ty: type_entity, .. } in iterated_returns {
                    let mut found = false;
                    for (ty_item, &ty_index) in types {
...
fn initialize_ords(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    ords: Vec<OgOrdEntity>,
) -> eyre::Result<HashMap<OgOrdEntity, NodeIndex>> {
...
fn connect_ords(
    graph: &mut StableGraph<SqlGraphEntity, SqlGraphRelationship>,
    ords: &HashMap<OgOrdEntity, NodeIndex>,
    schemas: &HashMap<SchemaEntity, NodeIndex>,
    types: &HashMap<OgTypeEntity, NodeIndex>,
    enums: &HashMap<OgEnumEntity, NodeIndex>,
    externs: &HashMap<OgExternEntity, NodeIndex>,
    ...
    for (item, &index) in ords {
        ...
        // Make OgOrdEntities (which will be translated into `CREATE OPERATOR CLASS` statements) depend
        // on the operators which they will reference. For example, a ogx-defined openGauss type `parakeet`
        // which has `#[derive(OgOrd)]` will emit a `parakeet_btree_ops` operator class, which references
...
fn initialize_hashes(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    hashes: Vec<OgHashEntity>,
) -> eyre::Result<HashMap<OgHashEntity, NodeIndex>> {
...
fn initialize_aggregates(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    aggregates: Vec<OgAggregateEntity>,
    mapped_builtin_types: &mut HashMap<String, NodeIndex>,
    mapped_enums: &HashMap<OgEnumEntity, NodeIndex>,
    mapped_types: &HashMap<OgTypeEntity, NodeIndex>,
) -> eyre::Result<HashMap<OgAggregateEntity, NodeIndex>> {
...
fn connect_aggregates(
    graph: &mut StableGraph<SqlGraphEntity, SqlGraphRelationship>,
    aggregates: &HashMap<OgAggregateEntity, NodeIndex>,
    schemas: &HashMap<SchemaEntity, NodeIndex>,
    types: &HashMap<OgTypeEntity, NodeIndex>,
    enums: &HashMap<OgEnumEntity, NodeIndex>,
    builtin_types: &HashMap<String, NodeIndex>,
...
fn initialize_triggers(
    ...
    bootstrap: Option<NodeIndex>,
    finalize: Option<NodeIndex>,
    triggers: Vec<OgTriggerEntity>,
) -> eyre::Result<HashMap<OgTriggerEntity, NodeIndex>> {
