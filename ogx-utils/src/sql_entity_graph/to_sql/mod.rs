...
use crate::sql_entity_graph::ogx_attribute::{ArgValue, OgxArg, OgxAttribute};
...
pub trait ToSql {
    ...
    ///
    /// Some entities require additional context from a [`OgxSql`], such as
    /// `#[derive(OgType)]` which must include it's relevant in/out functions.
...
/// A parsed `sql` option from a `ogx` related procedural macro.
...
const INVALID_ATTR_CONTENT: &str =
    "expected `#[ogx(sql = content)]`, where `content` is a boolean, string, or path to a function";
...
impl ToSqlConfig {
    ...
    pub fn from_attribute(attr: &Attribute) -> Result<Option<Self>, syn::Error> {
        if attr.style != AttrStyle::Outer {
            return Err(syn::Error::new(
                attr.span(),
                "#[ogx(sql = ..)] is only valid in an outer context",
            ));
        }

        let attr = attr.parse_args::<OgxAttribute>()?;
        for arg in attr.args.iter() {
            if let OgxArg::NameValue(ref nv) = arg {
    ...
    pub fn from_attributes(attrs: &[Attribute]) -> Result<Option<Self>, syn::Error> {
        if let Some(attr) = attrs.iter().find(|attr| attr.path.is_ident("ogx")) {
    ...
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        if let Some(callback_path) = callback {
            tokens.append_all(quote! {
                ::ogx::utils::sql_entity_graph::ToSqlConfigEntity {
                    enabled: #enabled,
