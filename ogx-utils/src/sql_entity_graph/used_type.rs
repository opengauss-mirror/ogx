...
to the `` framework and very subject to change between versions. While you may use this, please do it with caution.
...
impl UsedType {
            ...
            syn::Type::Macro(macro_pat) => {
                ...
                match archetype.ident.to_string().as_str() {
                    "variadic" => {
                        ...
                        syn::parse_quote! { ::ogx::VariadicArray<'static, #ty>}
                    }
    ...
    pub fn entity_tokens(&self) -> syn::Expr {
        ...
        syn::parse_quote! {
            ::ogx::utils::sql_entity_graph::UsedTypeEntity {
                ...
                optional: #optional,
                metadata: {
                    use ogx::utils::sql_entity_graph::metadata::PhantomDataExt;
                ...
                syn::Type::Macro(macro_pat) => {
                    ...
                    match archetype.ident.to_string().as_str() {
                        ...
                        "composite_type" => {
                            ...
                            let ty = syn::parse_quote! {
                                Vec<::ogx::heap_tuple::PgHeapTuple<'static, ::ogx::AllocatedByRust>>
                            };
                            Ok((ty, sql))
                        }
                    ...
                    syn::Type::Macro(macro_pat) => {
                        ...
                        match archetype.ident.to_string().as_str() {
                            ...
                            "composite_type" => {
                                let sql = Some(handle_composite_type_macro(mac)?);
                                let ty = syn::parse_quote! {
                                    ::ogx::VariadicArray<'static, ::ogx::heap_tuple::PgHeapTuple<'static, ::ogx::AllocatedByRust>>
                                };
                                Ok((ty, sql))
                            }
                            _ => Ok((syn::Type::Path(original), None)),
                    ...
                    syn::Type::Path(arg_type_path) => {
                        ...
                        match last.ident.to_string().as_str() {
                            "Option" => {
                                let (inner_ty, expr) = resolve_option_inner(arg_type_path)?;
                                let wrapped_ty = syn::parse_quote! {
                                    ::ogx::VariadicArray<'static, #inner_ty>
                                };
                                Ok((wrapped_ty, expr))
                            }
                            _ => Ok((syn::Type::Path(original), None)),
                    ...
                    syn::Type::Macro(macro_pat) => {
                        ...
                        match archetype.ident.to_string().as_str() {
                            ...
                            "composite_type" => {
                                let sql = Some(handle_composite_type_macro(mac)?);
                                let ty = syn::parse_quote! {
                                    ::ogx::Array<'static, ::ogx::heap_tuple::PgHeapTuple<'static, ::ogx::AllocatedByRust>>
                                };
                                Ok((ty, sql))
                            }
                            _ => Ok((syn::Type::Path(original), None)),
                    ...
                    syn::Type::Macro(macro_pat) => {
                        ...
                        match archetype.ident.to_string().as_str() {
                            ...
                            "composite_type" => {
                                let sql = Some(handle_composite_type_macro(mac)?);
                                let ty = syn::parse_quote! {
                                    Option<::ogx::heap_tuple::PgHeapTuple<'static, ::ogx::AllocatedByRust>>
                                };
                                Ok((ty, sql))
                            },
