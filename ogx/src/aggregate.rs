...
[`#[pg_aggregate]`](ogx_macros::pg_aggregate).
...
// ogx::pg_module_magic!(); // Uncomment this outside of docs!

#[derive(Copy, Clone, Default, OgType, Serialize, Deserialize)]
pub struct DemoSum {
    count: i32,
}
...
Functions inside the `impl` may use the [`#[ogx]`](macro@crate::ogx) attribute. It
accepts the same parameters as [`#[og_extern]`][macro@ogx-macros::og_extern].
...
use crate::error;
use crate::memcxt::OgMemoryContexts;
use crate::pg_sys::{AggCheckCallContext, CurrentMemoryContext, FunctionCallInfo, MemoryContext};
use crate::ogbox::OgBox;

pub use ogx_utils::sql_entity_graph::{FinalizeModify, ParallelOption};

/// Aggregate implementation trait.
///
/// When decorated with [`#[ogx_macros::pg_aggregate]`](ogx_macros::pg_aggregate), enables the
/// generation of [`CREATE AGGREGATE`](https://www.postgresql.org/docs/current/sql-createaggregate.html)
/// SQL.
///
/// The [`#[ogx_macros::pg_aggregate]`](ogx_macros::pg_aggregate) will automatically fill fields
    ...
    Self: Sized,
{
    ...
    ///
    /// Use [`ogx::name!()`](crate::name) to set the SQL name of the argument.
    ///
    /// If the final argument is to be variadic, use [`ogx::variadic`](crate::variadic). When used
    /// with [`ogx::name!()`](crate::name), it must be used **inside** the [`ogx::name!()`](crate::name) macro.
    type Args;
    ...
    ///
    /// `ogx` does not support `argname` as it is only used for documentation purposes.
    ///
    /// If the final argument is to be variadic, use `ogx::Variadic`.
    ///
    ...
    fn in_memory_context<
        R,
        F: FnOnce(&mut OgMemoryContexts) -> R + std::panic::UnwindSafe + std::panic::RefUnwindSafe,
    >(
        fcinfo: FunctionCallInfo,
        f: F,
    ) -> R {
        ...
        OgMemoryContexts::For(aggregate_memory_context).switch_to(f)
    }
}
