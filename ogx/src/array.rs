...
extern "C" {
    ...
    fn ogx_ARR_DATA_PTR(arrayType: *mut ArrayType) -> *mut u8;
    /// # Safety
    /// Does a field access, but doesn't deref out of bounds of ArrayType
    fn ogx_ARR_DIMS(arrayType: *mut ArrayType) -> *mut libc::c_int;
    /// # Safety
    /// Must only be used on a "valid" (Postgres-constructed) ArrayType
    fn ogx_ARR_NELEMS(arrayType: *mut ArrayType) -> i32;
    /// # Safety
    /// Does a field access, but doesn't deref out of bounds of ArrayType
    fn ogx_ARR_NULLBITMAP(arrayType: *mut ArrayType) -> *mut bits8;
}
...
OGX currently only intentionally supports 64-bit machines,
and while support for ILP32 or I64LP128 C data models may become possible,
OGX will **not** support 16-bit machines in any practical case, even though Rust does.
...
impl RawArray {
    ...
    */
    pub unsafe fn from_ptr(ptr: NonNull<ArrayType>) -> RawArray {
        ...
        let len = unsafe { ogx_ARR_NELEMS(ptr.as_ptr()) } as usize;
        RawArray { ptr, len }
    }
    ...
    pub unsafe fn from_array<T: FromDatum>(arr: Array<T>) -> Option<RawArray> {
        ...
        // SAFETY: Validity asserted by the caller.
        let len = unsafe { ogx_ARR_NELEMS(array_type) } as usize;
    ...
    fn ndim(&self) -> libc::c_int {
        ...
        unsafe {
            ...
            however, OGX has trouble with that, unfortunately.
            */
            as _
        }
    }
    ...
    pub fn dims(&self) -> &[libc::c_int] {
        ...
        */
        unsafe {
            ...
            slice::from_raw_parts(ogx_ARR_DIMS(self.ptr.as_ptr()), ndim)
        }
    }
    ...
    */
    ...
    fn nulls_mut_ptr(&mut self) -> *mut u8 {
        ...
        unsafe { ogx_ARR_NULLBITMAP(self.ptr.as_ptr()) }
    }
        ...
        */
        unsafe {
            NonNull::new_unchecked(slice_from_raw_parts_mut(
                ogx_ARR_DATA_PTR(self.ptr.as_ptr()).cast(),
                self.len,
            ))
        }
    }
