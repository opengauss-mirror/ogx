...
pub enum OgXactCallbackEvent {
    /// Fired when a transaction is aborted.  It is mutually exclusive with `OgXactCallbackEvent::Commit`
    ///
    /// ## Safety
    ///
    ...
    /// Fired when a transcation is committed.  It is mutually exclusive with `OgXactCallbackEvent::Abort`
    ///
    /// ## Safety
    ///
    /// Any kind of Rust `panic!()` or Postgres `ereport(ERROR)` while this event is firing will
...
impl OgXactCallbackEvent {
    fn translate_pg_event(pg_event: pg_sys::XactEvent) -> Self {
        match pg_event {
            pg_sys::XactEvent_XACT_EVENT_ABORT => OgXactCallbackEvent::Abort,
            pg_sys::XactEvent_XACT_EVENT_COMMIT => OgXactCallbackEvent::Commit,
            pg_sys::XactEvent_XACT_EVENT_PARALLEL_ABORT => OgXactCallbackEvent::ParallelAbort,
            pg_sys::XactEvent_XACT_EVENT_PARALLEL_COMMIT => OgXactCallbackEvent::ParallelCommit,
            pg_sys::XactEvent_XACT_EVENT_PARALLEL_PRE_COMMIT => {
                OgXactCallbackEvent::ParallelPreCommit
            }
            pg_sys::XactEvent_XACT_EVENT_PREPARE => OgXactCallbackEvent::Prepare,
            pg_sys::XactEvent_XACT_EVENT_PRE_COMMIT => OgXactCallbackEvent::PreCommit,
            pg_sys::XactEvent_XACT_EVENT_PRE_PREPARE => OgXactCallbackEvent::PrePrepare,
...
impl XactCallbackReceipt {
    ...
    /// use ogx::*;
    ///
    /// let receipt = register_xact_callback(OgXactCallbackEvent::Commit, || info!("called after commit"));
    ///
...
type CallbackMap = HashMap<OgXactCallbackEvent, Vec<Rc<RefCell<Option<XactCallbackWrapper>>>>>;

/// Register a closure to be called during one of the `OgXactCallbackEvent` events.  Multiple
...
/// use ogx::*;
///
/// register_xact_callback(OgXactCallbackEvent::PreCommit, || info!("pre-commit #1"));
/// register_xact_callback(OgXactCallbackEvent::PreCommit, || info!("pre-commit #2"));
/// register_xact_callback(OgXactCallbackEvent::PreCommit, || info!("pre-commit #3"));
/// register_xact_callback(OgXactCallbackEvent::Commit, || info!("called after commit"));
/// ```
///
...
///
/// let receipt = register_xact_callback(OgXactCallbackEvent::Abort, || { /* do cleanup if xact aborts */});
///
...
/// ## Safety
///
/// Any kind of Rust `panic!()` or Postgres `ereport(ERROR)` while executing a `OgXactCallbackEvent::Commit`
/// or `OgXactCallbackEvent::Abort` event will immediately cause the Postgres backend to abort and
...
pub fn register_xact_callback<F>(which_event: OgXactCallbackEvent, f: F) -> XactCallbackReceipt
where
    F: FnOnce() + std::panic::UnwindSafe + std::panic::RefUnwindSafe + 'static,
{
    ...
    unsafe extern "C" fn callback(event: pg_sys::XactEvent, _arg: *mut ::std::os::raw::c_void) {
        let which_event = OgXactCallbackEvent::translate_pg_event(event);

        let hooks = match which_event {
            // ogx's XactCallbacks are per-transaction, so when the transaction is over
            ...
            OgXactCallbackEvent::Commit
            | OgXactCallbackEvent::Abort
            | OgXactCallbackEvent::ParallelCommit
            | OgXactCallbackEvent::ParallelAbort => XACT_HOOKS
                .replace(HashMap::new())
    ...
    fn maybe_initialize<'a>() -> &'a mut SubCallbackMap {
        unsafe {
            if SUB_HOOKS.is_none() {
                ...
                register_xact_callback(OgXactCallbackEvent::Commit, || {
                    // reset SUB_HOOKS to None on outer transaction COMMIT
                    SUB_HOOKS.take();
                });
                register_xact_callback(OgXactCallbackEvent::Abort, || {
