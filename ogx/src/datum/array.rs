...
impl<'a, T: FromDatum> Drop for Array<'a, T> {
    fn drop(&mut self) {
        if let Array { ptr, raw, datum_palloc: Some(data), elem_slice, .. } = self {
            ...
            let ptr = ptr.take();
            match (ptr, raw) {
                // SAFETY: if ogx detoasted a clone of this varlena, pfree the clone
...
impl<'a, T: FromDatum> Array<'a, T> {
    ...
    unsafe fn deconstruct_from(
        ...
        */
        ...
        // But ogx doesn't actually need [bool] if NullKind's handling of BitSlices is correct.
    ...
    T: IntoDatum,
{
    fn into_datum(self) -> Option<pg_sys::Datum> {
        ...
        if state.is_null() {
            ...
            Some(unsafe {
                pg_sys::makeArrayResult(state, OgMemoryContexts::CurrentMemoryContext.value())
            })
        }
    }
