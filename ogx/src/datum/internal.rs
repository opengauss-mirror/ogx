...
use crate::{pg_sys, FromDatum, IntoDatum, OgMemoryContexts};
...
impl Internal {
    ...
    ///
    /// The value will be dropped when the [OgMemoryContexts::CurrentMemoryContext] is deleted.
    #[inline(always)]
    pub fn new<T>(t: T) -> Self {
        Self(Some(pg_sys::Datum::from(
            OgMemoryContexts::CurrentMemoryContext.leak_and_drop_on_delete(t),
        )))
    }
    ...
    pub unsafe fn insert<T>(&mut self, value: T) -> &mut T {
        let datum = pg_sys::Datum::from(
            OgMemoryContexts::CurrentMemoryContext.leak_and_drop_on_delete(value),
        );
        ...
        F: FnOnce() -> T,
    {
        let ptr = self.0.get_or_insert_with(|| {
            let result = f();
            let datum =
                OgMemoryContexts::CurrentMemoryContext.leak_and_drop_on_delete(result).into();
            datum
        });
        &mut *(ptr.cast_mut_ptr::<T>())
    }
