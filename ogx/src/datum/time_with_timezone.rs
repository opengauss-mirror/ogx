...
use crate::{pg_sys, FromDatum, IntoDatum, OgBox};
...
impl FromDatum for TimeWithTimeZone {
    ...
    unsafe fn from_polymorphic_datum(
        ...
        if is_null {
            ...
            let timetz = OgBox::from_pg(datum.cast_mut_ptr::<pg_sys::TimeTzADT>());
...
impl IntoDatum for TimeWithTimeZone {
    ...
    fn into_datum(self) -> Option<pg_sys::Datum> {
        let mut timetz = OgBox::<pg_sys::TimeTzADT>::alloc();
...
impl TimeWithTimeZone {
    ...
    #[deprecated(
        since = "0.5.0",
        note = "the repr of ogx::TimeWithTimeZone is no longer time::Time \
