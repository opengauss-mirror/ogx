...
use crate::{
    ...
    varsize_any_exhdr, void_mut_ptr, FromDatum, IntoDatum, OgMemoryContexts, OgType,
    StringInfo,
};
use ogx_pg_sys::varlena;
...
impl Clone for PallocdVarlena {
    fn clone(&self) -> Self {
        ...
        let ptr = unsafe {
            OgMemoryContexts::Of(self.ptr as void_mut_ptr)
...
///
/// Generally, `PgVarlena` is meant to be used in conjunction with ogx's `OgType` derive macro
...
///     fn input(input: &ogx::cstr_core::CStr) -> PgVarlena<Self> {
    ...
    T: OgType + Deserialize<'de>,
{
    ...
    unsafe fn from_datum_in_memory_context(
        memory_context: OgMemoryContexts,
        datum: pg_sys::Datum,
        is_null: bool,
        _typoid: u32,
    ) -> Option<Self> {
