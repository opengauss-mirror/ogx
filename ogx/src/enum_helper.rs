...
use crate::pg_sys::ogx_GETSTRUCT;
...
pub fn lookup_enum_by_oid(enumval: pg_sys::Oid) -> (String, pg_sys::Oid, f32) {
    ...
    let en = unsafe { ogx_GETSTRUCT(tup) } as pg_sys::Form_pg_enum;
...
unsafe fn extract_enum_oid(tup: *mut pg_sys::HeapTupleData) -> pg_sys::Oid {
    extern "C" {
        fn ogx_HeapTupleHeaderGetOid(htup_header: pg_sys::HeapTupleHeader) -> pg_sys::Oid;
    }

    ogx_HeapTupleHeaderGetOid(tup.as_ref().unwrap().t_data)
}
