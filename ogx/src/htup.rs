...
pub fn composite_row_type_make_tuple(
    row: pg_sys::Datum,
) -> OgBox<pg_sys::HeapTupleData, AllocatedByRust> {
    let htup_header =
        unsafe { pg_sys::pg_detoast_datum_packed(row.cast_mut_ptr()) } as pg_sys::HeapTupleHeader;
    let mut tuple = OgBox::<pg_sys::HeapTupleData>::alloc0();
...
>(
    ...
    let datum = unsafe {
        ogx_heap_getattr(tuple.as_ptr(), attno.get() as u32, tupdesc.as_ptr(), &mut is_null)
    };
...
pub unsafe fn heap_getattr_raw(
    ...
    let mut is_null = false;
    let datum = ogx_heap_getattr(tuple, attno.get() as u32, tupdesc, &mut is_null);
    if is_null {
        None
    } else {
        Some(datum)
...
pub fn heap_getattr_datum_ex(
    tuple: &OgBox<pg_sys::HeapTupleData>,
    attno: usize,
    tupdesc: &PgTupleDesc,
) -> DatumWithTypeInfo {
    let mut is_null = false;
    let datum =
        unsafe { (tuple.as_ptr(), attno as u32, tupdesc.as_ptr(), &mut is_null) };
