...
//! Helper trait for the `#[derive(OgType)]` proc macro for overriding custom Postgres type
...
/// `#[derive(Copy, Clone, OgType)]` types need to implement this trait to provide the text
...
/// `#[derive(Serialize, Deserialize, OgType)]` types may implement this trait if they prefer
...
/// Automatically implemented for `#[derive(Serialize, Deserialize, OgType)]` types that do
