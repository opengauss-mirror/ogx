/*!
Code for interfacing with the layout in memory (and on disk?) of various data types within openGauss.
Prefer to minimize exposing the contents of this module to public callers of the ogx library:
This is not a mature module yet so prefer to avoid exposing the contents to public callers of ogx,
...
Some may be better off extending the ogx bindings themselves, doing their own layout checks, etc.
When OGX is a bit more mature in its soundness, and we better understand what our callers expect,
