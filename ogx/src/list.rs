...
impl<T> PgList<T> {
    ...
    pub fn head(&self) -> Option<*mut T> {
        if self.list.is_null() {
            ...
            Some(unsafe { pg_sys::ogx_list_nth(self.list, 0) } as *mut T)
        }
    }
    ...
    pub fn tail(&self) -> Option<*mut T> {
        if self.list.is_null() {
            ...
            Some(unsafe { pg_sys::ogx_list_nth(self.list, (self.len() - 1) as i32) } as *mut T)
        }
    }
    ...
    pub fn get_ptr(&self, i: usize) -> Option<*mut T> {
        ...
        if self.list.is_null() || i >= self.len() {
            ...
            Some(unsafe { pg_sys::ogx_list_nth(self.list, i as i32) } as *mut T)
        }
    }
    ...
    pub fn get_int(&self, i: usize) -> Option<i32> {
        ...
        if self.list.is_null() || i >= self.len() {
            ...
            Some(unsafe { pg_sys::ogx_list_nth_int(self.list, i as i32) })
        }
    }
    ...
    pub fn get_oid(&self, i: usize) -> Option<pg_sys::Oid> {
        ...
        if self.list.is_null() || i >= self.len() {
            ...
            Some(unsafe { pg_sys::ogx_list_nth_oid(self.list, i as i32) })
        }
    }
    ...
    pub unsafe fn replace_ptr(&mut self, i: usize, with: *mut T) {
        let cell = pg_sys::ogx_list_nth_cell(self.list, i as i32);
