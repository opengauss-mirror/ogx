...
pub fn elog(level: PgLogLevel, message: &str) {
    ...
    unsafe {
        extern "C" {
            fn ogx_elog(level: i32, message: *const c_char);
        }

        match CString::new(message) {
            Ok(s) => crate::guard(|| ogx_elog(level as i32, s.as_ptr())),
            Err(_) => crate::guard(|| {
                ogx_elog(
                    level as i32,
...
///
///     for i in input {
///         ogx::info!("i={index:?}, sum={}", sum, index = i);
...
///
///     for i in input {
///         ogx::notice!("i={index:?}, sum={}", sum, index = i);
///         sum += i.unwrap_or(-1) as i64;
///     }
///
///     sum
...
///
///     for i in input {
///         ogx::warning!("i={index:?}, sum={}", sum, index = i);
///         sum += i.unwrap_or(-1) as i64;
///     }
///
///     sum
...
///
///     for i in input {
///         ogx::error!("i={index:?}, sum={}", sum, index = i);
///         sum += i.unwrap_or(-1) as i64;
///     }
///
///     sum
...
///
///     for i in input {
///         ogx::FATAL!("i={index:?}, sum={}", sum, index = i);
///         sum += i.unwrap_or(-1) as i64;
///     }
///
///     sum
...
///
///     for i in input {
///         ogx::PANIC!("i={index:?}, sum={}", sum, index = i);
///         sum += i.unwrap_or(-1) as i64;
///     }
///
///     sum
...
/// Emit a test output message for highlighting during `cargo ogx test`
#[cfg(test)]
#[macro_export]
macro_rules! testmsg {
    ($($arg:tt)*) => (
