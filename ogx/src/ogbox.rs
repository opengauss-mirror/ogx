...
/// Similar to Rust's `Box<T>` type, `OgBox<T>` also represents heap-allocated memory.
use crate::{pg_sys, OgMemoryContexts};
//use std::fmt::{Debug, Error, Formatter};
use ogx_utils::sql_entity_graph::metadata::{
...
/// allocation functions (`palloc`, etc).  Think of `OgBox<T>` as a wrapper around an otherwise
...
/// backing Postgres-allocated memory is `pfree()'d` when the `OgBox<T>` is dropped, but it is
...
///     let mut tid = OgBox::<pg_sys::ItemPointerData>::alloc();
///
...
/// ```
///
/// A similar example, but instead the `OgBox<T>`'s backing memory gets freed when the box is
/// dropped:
///
/// ```rust,no_run
/// use ogx::*;
...
///     let relation = unsafe { OgBox::from_pg(pg_sys::relation_open(relid, lockmode)) };
///
...
///
///     // While the `OgBox` instance gets dropped, the backing Postgres-allocated pointer is
///     // **not** freed since it came "::from_pg()".  We don't own the underlying memory so
///     // we can't free it
/// }
/// ```
#[repr(transparent)]
pub struct OgBox<T, AllocatedBy: WhoAllocated<T> = AllocatedByOpenGauss> {
    ptr: Option<NonNull<T>>,
    __marker: PhantomData<AllocatedBy>,
}

/// A trait to track if the contents of a [OgBox] were allocated by Rust or Postgres.
pub trait WhoAllocated<T> {
    fn free(ptr: *mut T);
}

/// Indicates the [OgBox] contents were allocated by Postgres.  This is also OgBox' default
/// understanding.
pub struct AllocatedByOpenGauss;

/// Indicates the [OgBox] contents were allocated by Rust.
pub struct AllocatedByRust;

impl<T> WhoAllocated<T> for AllocatedByOpenGauss {
    /// Doesn't do anything
    fn free(_ptr: *mut T) {}
}
...
impl<T> OgBox<T, AllocatedByOpenGauss> {
    /// Box a pointer that cames from Postgres.
    ///
    /// When this `OgBox<T>` is dropped, the boxed memory is **not** freed.  Since Postgres
    /// allocated it, Postgres is responsible for freeing it.
    #[inline]
    pub unsafe fn from_pg(ptr: *mut T) -> OgBox<T, AllocatedByOpenGauss> {
        OgBox::<T, AllocatedByOpenGauss> { ptr: NonNull::new(ptr), __marker: PhantomData }
    }
}

impl<T> OgBox<T, AllocatedByRust> {
    /**
    ...
    If you need to give the boxed pointer to Postgres, call [`.into_pg()`][OgBox::into_pg].

    ```rust,no_run
    use ogx::{OgBox, AllocatedByRust};
    ...
    ```
    */
    pub fn new(val: T) -> OgBox<T, AllocatedByRust> {
        let ptr = Self::alloc0();
    ...
    ```rust,no_run
    use ogx::{OgBox, OgMemoryContexts, AllocatedByRust};

    let ptr: OgBox<i32, AllocatedByRust> = OgBox::new_in_context(5, OgMemoryContexts::CurrentMemoryContext);
    assert_eq!(*ptr, 5);
    ```
    */
    pub fn new_in_context(val: T, memory_context: OgMemoryContexts) -> OgBox<T, AllocatedByRust> {
...
impl<T, AllocatedBy: WhoAllocated<T>> OgBox<T, AllocatedBy> {
    /// Box a pointer that was allocated within Rust
    ///
    /// When this `OgBox<T>` is dropped, the boxed memory is freed.  Since Rust
    /// allocated it, Rust is responsible for freeing it.
    ///
    /// If you need to give the boxed pointer to Postgres, call [`.into_pg()`][OgBox::into_pg]
    #[inline]
    pub unsafe fn from_rust(ptr: *mut T) -> OgBox<T, AllocatedByRust> {
        OgBox::<T, AllocatedByRust> { ptr: NonNull::new(ptr), __marker: PhantomData }
    }
    ...
    /// ## Examples
    /// ```rust,no_run
    /// use ogx::{OgBox, pg_sys};
    /// let ctid = OgBox::<pg_sys::ItemPointerData>::alloc();
    /// ```
    #[inline]
    pub fn alloc() -> OgBox<T, AllocatedByRust> {
        OgBox::<T, AllocatedByRust> {
            ptr: Some(unsafe {
    ...
    /// ```rust,no_run
    /// use ogx::{OgBox, pg_sys};
    /// let ctid = OgBox::<pg_sys::ItemPointerData>::alloc0();
    /// ```
    #[inline]
    pub fn alloc0() -> OgBox<T, AllocatedByRust> {
        OgBox::<T, AllocatedByRust> {
            ptr: Some(unsafe {
    ...
    /// ## Examples
    /// ```rust,no_run
    /// use ogx::{OgBox, pg_sys, OgMemoryContexts};
    /// let ctid = OgBox::<pg_sys::ItemPointerData>::alloc_in_context(OgMemoryContexts::TopTransactionContext);
    /// ```
    #[inline]
    pub fn alloc_in_context(memory_context: OgMemoryContexts) -> OgBox<T, AllocatedByRust> {
        OgBox::<T, AllocatedByRust> {
            ptr: Some(unsafe {
    ...
    /// ```rust,no_run
    /// use ogx::{OgBox, pg_sys, OgMemoryContexts};
    /// let ctid = OgBox::<pg_sys::ItemPointerData>::alloc0_in_context(OgMemoryContexts::TopTransactionContext);
    /// ```
    #[inline]
    pub fn alloc0_in_context(memory_context: OgMemoryContexts) -> OgBox<T, AllocatedByRust> {
        OgBox::<T, AllocatedByRust> {
            ptr: Some(unsafe {
    ...
    /// ```rust,no_run
    /// use ogx::{OgBox, pg_sys};
    /// let create_trigger_statement = OgBox::<pg_sys::CreateTrigStmt>::alloc_node(pg_sys::NodeTag_T_CreateTrigStmt);
    /// ```
    #[inline]
    pub fn alloc_node(node_tag: pg_sys::NodeTag) -> OgBox<T, AllocatedByRust> {
        let node = OgBox::<T>::alloc0();
        let ptr = node.as_ptr();
    ...
    /// Box nothing
    #[inline]
    pub fn null() -> OgBox<T, AllocatedByOpenGauss> {
        OgBox::<T, AllocatedByOpenGauss> { ptr: None, __marker: PhantomData }
    }
    ...
    /// The boxed pointer is **not** free'd by Rust
    #[inline]
    pub fn into_pg_boxed(mut self) -> OgBox<T, AllocatedByOpenGauss> {
        // SAFETY:  we know our internal pointer is good so we can now make it owned by Postgres
        unsafe {
            OgBox::from_pg(match self.ptr.take() {
                Some(ptr) => ptr.as_ptr(),
                None => std::ptr::null_mut(),
            })
        }
    ...
    /// Execute a closure with a mutable, `OgBox`'d form of the specified `ptr`
    #[inline]
    pub unsafe fn with<F: FnOnce(&mut OgBox<T>)>(ptr: *mut T, func: F) {
        func(&mut OgBox::from_pg(ptr))
    }
}

impl<T, AllocatedBy: WhoAllocated<T>> Deref for OgBox<T, AllocatedBy> {
    type Target = T;

    fn deref(&self) -> &Self::Target {
        match self.ptr.as_ref() {
            Some(ptr) => unsafe { ptr.as_ref() },
            None => panic!("Attempt to dereference null pointer during Deref of OgBox"),
        }
    }
}

impl<T, AllocatedBy: WhoAllocated<T>> DerefMut for OgBox<T, AllocatedBy> {
    fn deref_mut(&mut self) -> &mut T {
        match self.ptr.as_mut() {
            Some(ptr) => unsafe { ptr.as_mut() },
            None => panic!("Attempt to dereference null pointer during DerefMut of OgBox"),
        }
    }
}

impl<T, AllocatedBy: WhoAllocated<T>> Drop for OgBox<T, AllocatedBy> {
    fn drop(&mut self) {
...
unsafe impl<T: SqlTranslatable> SqlTranslatable for OgBox<T, AllocatedByOpenGauss> {
...
unsafe impl<T: SqlTranslatable> SqlTranslatable for OgBox<T, AllocatedByRust> {
    fn argument_sql() -> Result<SqlMapping, ArgumentError> {
        T::argument_sql()
    }
    fn return_sql() -> Result<Returns, ReturnsError> {
