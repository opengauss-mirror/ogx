...
use crate::{
    direct_function_call, name_data_to_str, pg_sys, FromDatum, IntoDatum, OgBox, PgList,
    PgTupleDesc,
};
...
pub struct PgRelation {
    boxed: OgBox<pg_sys::RelationData>,
    need_close: bool,
...
impl PgRelation {
    ...
    pub unsafe fn from_pg(ptr: pg_sys::Relation) -> Self {
        PgRelation { boxed: OgBox::from_pg(ptr), need_close: false, lockmode: None }
    }
    ...
    pub unsafe fn from_pg_owned(ptr: pg_sys::Relation) -> Self {
        PgRelation { boxed: OgBox::from_pg(ptr), need_close: true, lockmode: None }
    }
    ...
    pub unsafe fn open(oid: pg_sys::Oid) -> Self {
        ...
        PgRelation { boxed: OgBox::from_pg(rel), need_close: true, lockmode: None }
    }
    ...
    pub fn with_lock(oid: pg_sys::Oid, lockmode: pg_sys::LOCKMODE) -> Self {
        unsafe {
            PgRelation {
                boxed: OgBox::from_pg(pg_sys::relation_open(oid, lockmode)),
                need_close: true,
    ...
    pub fn namespace_oid(&self) -> pg_sys::Oid {
        ...
        let rd_rel: OgBox<pg_sys::FormData_pg_class> = unsafe { OgBox::from_pg(self.boxed.rd_rel) };
        rd_rel.relnamespace
    }
    ...
    pub fn heap_relation(&self) -> Option<PgRelation> {
        // SAFETY: we know self.boxed and its members are correct as we created it
        let rd_index: OgBox<pg_sys::FormData_pg_index> =
            unsafe { OgBox::from_pg(self.boxed.rd_index) };
        if rd_index.is_null() {
            None
        } else {
...
impl Deref for PgRelation {
    type Target = OgBox<pg_sys::RelationData>;
