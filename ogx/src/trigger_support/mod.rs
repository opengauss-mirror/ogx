...
```rust,no_run
use ogx::{og_trigger, pg_sys, heap_tuple::{PgHeapTuple, PgHeapTupleError}, WhoAllocated, OgTrigger};

#[og_trigger]
fn trigger_example(trigger: &OgTrigger) -> Result<
...
Trigger functions only accept one argument, a [`OgTrigger`], and they return a [`Result`][std::result::Result] containing
...
```sql
-- ogx-examples/triggers/src/lib.rs:25
...
or [`AllocatedByOpenGauss`][crate::AllocatedByOpenGauss]. In most cases, it can be inferred by the compiler using
...
```rust,no_run
use ogx::{og_trigger, pg_sys, heap_tuple::{PgHeapTuple, PgHeapTupleError}, WhoAllocated, AllocatedByRust, AllocatedByOpenGauss, OgTrigger};

#[og_trigger]
fn example_allocated_by_rust(trigger: &OgTrigger) -> Result<
...
#[og_trigger]
fn example_allocated_by_postgres(trigger: &OgTrigger) -> Result<
    PgHeapTuple<'_, AllocatedByOpenGauss>,
    PgHeapTupleError,
> {
    let current = unsafe { trigger.current() }.expect("No current HeapTuple");
    Ok(current)
...
enum CustomTriggerError {
    ...
    NoCurrentHeapTuple,
    #[error("ogx::PgHeapTupleError: {0}")]
    PgHeapTuple(PgHeapTupleError),
}

#[og_trigger]
fn example_custom_error(trigger: &OgTrigger) -> Result<
    PgHeapTuple<'_, impl WhoAllocated<pg_sys::HeapTupleData>>,
    CustomTriggerError,
> {
...
```rust,no_run
use ogx::{og_trigger, pg_sys, heap_tuple::{PgHeapTuple, PgHeapTupleError}, WhoAllocated, AllocatedByRust, OgTrigger};
...
#[og_trigger]
fn example_lifetimes<'a, 'b>(trigger: &'a OgTrigger) -> Result<
...
Unsafe [`ogx::pg_sys::FunctionCallInfo`][crate::pg_sys::FunctionCallInfo] and
[`ogx::pg_sys::TriggerData`][crate::pg_sys::TriggerData] (include its contained
[`ogx::pg_sys::Trigger`][crate::pg_sys::Trigger]) accessors are available..


# Getting safe data all at once

Many [`OgTrigger`][OgTrigger] functions are `unsafe` as they dereference pointers inside the
[`TriggerData`][crate::pg_sys::TriggerData] contained by the [`OgTrigger`][OgTrigger].

In cases where a safe API is desired, the [`OgTriggerSafe`] structure can be retrieved
from [`OgTrigger::to_safe`].

```rust,no_run
use ogx::{og_trigger, pg_sys, heap_tuple::{PgHeapTuple, PgHeapTupleError}, WhoAllocated, OgTrigger, OgTrigger};

#[og_trigger]
fn trigger_safe(trigger: &OgTrigger) -> Result<
    PgHeapTuple<'_, impl WhoAllocated<pg_sys::HeapTupleData>>,
    OgTriggerError,
> {
...
mod og_trigger;
mod og_trigger_error;
mod og_trigger_level;
mod og_trigger_option;
mod og_trigger_safe;
mod og_trigger_when;
mod trigger_tuple;

pub use og_trigger::OgTrigger;
pub use og_trigger_error::OgTriggerError;
pub use og_trigger_level::OgTriggerLevel;
pub use og_trigger_option::OgTriggerOperation;
pub use og_trigger_safe::OgTriggerSafe;
pub use og_trigger_when::OgTriggerWhen;
