...
use crate::pg_sys;
use crate::ogbox::{AllocatedByOpenGauss, OgBox};
use crate::rel::PgRelation;
use crate::trigger_support::{
    called_as_trigger, OgTriggerError, OgTriggerLevel, OgTriggerOperation, OgTriggerSafe,
    OgTriggerWhen, TriggerEvent, TriggerTuple,
};
...
pub struct OgTrigger {
    ...
    trigger_data: OgBox<ogx_pg_sys::TriggerData>,
...
impl OgTrigger {
    /// Construct a new [`OgTrigger`] from a [`FunctionCallInfo`][pg_sys::FunctionCallInfo]
    ///
    /// Generally this would be automatically done for the user in a [`#[og_trigger]`][crate::og_trigger].
    ///
    /// # Safety
    ///
    ...
    ///
    /// * one provided by openGauss during a trigger invocation,
    /// * unharmed (the user has not mutated it since openGauss provided it),
    ///
    /// If any of these conditions are untrue, this or any other function on this type is
    /// undefined behavior, hopefully panicking.
    pub unsafe fn from_fcinfo(fcinfo: pg_sys::FunctionCallInfo) -> Result<Self, OgTriggerError> {
        if fcinfo.is_null() {
            return Err(OgTriggerError::NullFunctionCallInfo);
        }
        if !called_as_trigger(fcinfo) {
            return Err(OgTriggerError::NotTrigger);
        }
        let fcinfo_data = &*fcinfo;

        if fcinfo_data.context.is_null() {
            return Err(OgTriggerError::NullTriggerData);
        }
        let trigger_data: OgBox<pg_sys::TriggerData> =
            OgBox::from_pg(fcinfo_data.context as *mut pg_sys::TriggerData);

        let trigger_ptr = trigger_data.tg_trigger;
        if trigger_ptr.is_null() {
            return Err(OgTriggerError::NullTrigger);
        }
        let trigger = *trigger_ptr;

        let relation_data_ptr = trigger_data.tg_relation;
        if relation_data_ptr.is_null() {
            return Err(OgTriggerError::NullRelation);
        }
    ...
    /// The new HeapTuple
    // Derived from `ogx_pg_sys::TriggerData.tg_newtuple` and `ogx_pg_sys::TriggerData.tg_newslot.tts_tupleDescriptor`
    pub fn new(&self) -> Option<PgHeapTuple<'_, AllocatedByOpenGauss>> {
    ...
    // Derived from `ogx_pg_sys::TriggerData.tg_trigtuple` and `ogx_pg_sys::TriggerData.tg_trigslot.tts_tupleDescriptor`
    pub fn current(&self) -> Option<PgHeapTuple<'_, AllocatedByOpenGauss>> {
        // Safety: Given that we have a known good `FunctionCallInfo`, which PostgreSQL has checked is indeed a trigger,
        // containing a known good `TriggerData` which also contains a known good `Trigger`... and the user aggreed to
        // our `unsafe` constructor safety rules, we choose to trust this is indeed a valid pointer offered to us by
        // PostgreSQL, and that it trusts it.
    ...
    pub fn name(&self) -> Result<&str, OgTriggerError> {
    ...
    // Derived from `ogx_pg_sys::TriggerData.tg_event`
    pub fn when(&self) -> Result<OgTriggerWhen, OgTriggerError> {
        OgTriggerWhen::try_from(TriggerEvent(self.trigger_data.tg_event))
    }
    /// The level, from the trigger definition (`ROW`, `STATEMENT`)
    // Derived from `ogx_pg_sys::TriggerData.tg_event`
    pub fn level(&self) -> OgTriggerLevel {
        OgTriggerLevel::from(TriggerEvent(self.trigger_data.tg_event))
    }
    /// The operation for which the trigger was fired
    // Derived from `ogx_pg_sys::TriggerData.tg_event`
    pub fn op(&self) -> Result<OgTriggerOperation, OgTriggerError> {
        OgTriggerOperation::try_from(TriggerEvent(self.trigger_data.tg_event))
    }
    /// the object ID of the table that caused the trigger invocation
    // Derived from `ogx_pg_sys::TriggerData.tg_relation.rd_id`
    pub fn relid(&self) -> Result<pg_sys::Oid, OgTriggerError> {
    ...
    // Derived from `ogx_pg_sys::TriggerData.trigger.tgoldtable`
    pub fn old_transition_table_name(&self) -> Result<Option<&str>, OgTriggerError> {
    ...
    // Derived from `ogx_pg_sys::TriggerData.trigger.tgoldtable`
    pub fn new_transition_table_name(&self) -> Result<Option<&str>, OgTriggerError> {
    ...
    pub unsafe fn relation(&self) -> Result<crate::PgRelation, OgTriggerError> {
    ...
    ///
    /// As such, this function is unsafe as we cannot guarantee that this requirement is true.
    pub unsafe fn table_name(&self) -> Result<String, OgTriggerError> {
    ...
    ///
    /// As such, this function is unsafe as we cannot guarantee that this requirement is true.
    pub unsafe fn table_schema(&self) -> Result<String, OgTriggerError> {
        let relation = self.relation()?;
        Ok(relation.namespace().to_string())
    }
    /// The arguments from the CREATE TRIGGER statement
    // Derived from `ogx_pg_sys::TriggerData.trigger.tgargs`
    pub fn extra_args(&self) -> Result<Vec<String>, OgTriggerError> {
    ...
    /// A reference to the underlaying [`RelationData`][ogx_pg_sys::RelationData]
    pub fn relation_data(&self) -> &ogx_pg_sys::RelationData {
        self.relation_data.borrow()
    }

    /// A reference to the underlaying [`Trigger`][ogx_pg_sys::Trigger]
    pub fn trigger(&self) -> &ogx_pg_sys::Trigger {
        self.trigger.borrow()
    }

    /// A reference to the underlaying [`TriggerData`][ogx_pg_sys::TriggerData]
    pub fn trigger_data(&self) -> &ogx_pg_sys::TriggerData {
    ...
    /// Eagerly evaluate the data in this `OgTrigger` and build a safely accessible structure
    /// which mimics the data provided to a PL/pgSQL trigger.
    pub unsafe fn to_safe(&self) -> Result<OgTriggerSafe, OgTriggerError> {
        let trigger_safe = OgTriggerSafe {
            name: self.name()?,
            new: self.new(),
