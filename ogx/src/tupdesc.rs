...
use crate::{pg_sys, void_mut_ptr, OgBox, PgRelation};

use ogx_pg_sys::AsPgCStr;
use std::ops::Deref;
...
///
/// OGX's safe wrapper takes care of properly freeing or decrementing reference counts
pub struct PgTupleDesc<'a> {
    tupdesc: Option<OgBox<pg_sys::TupleDescData>>,
...
impl<'a> PgTupleDesc<'a> {
    ...
    pub unsafe fn from_pg<'b>(ptr: pg_sys::TupleDesc) -> PgTupleDesc<'b> {
        PgTupleDesc {
            tupdesc: Some(OgBox::from_pg(ptr)),
            parent: None,
            need_release: true,
            need_pfree: false,
        }
    ...
    pub unsafe fn from_pg_copy<'b>(ptr: pg_sys::TupleDesc) -> PgTupleDesc<'b> {
        PgTupleDesc {
            ...
            tupdesc: Some(OgBox::from_pg(pg_sys::CreateTupleDescCopyConstr(ptr))),
            parent: None,
            need_release: false,
            need_pfree: true,
        }
    ...
    pub fn from_relation(parent: &PgRelation) -> PgTupleDesc {
        PgTupleDesc {
            ...
            tupdesc: Some(unsafe { OgBox::from_pg(parent.rd_att) }),
            parent: Some(parent),
            need_release: false,
            need_pfree: false,
        }
...
impl<'a> Deref for PgTupleDesc<'a> {
    type Target = OgBox<pg_sys::TupleDescData>;
...
#[inline]
fn tupdesc_get_attr(
    tupdesc: &OgBox<pg_sys::TupleDescData>,
    attno: usize,
