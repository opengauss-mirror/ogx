...
use crate::{pg_sys, OgBox};

pub unsafe fn set_varsize(ptr: *mut pg_sys::varlena, len: i32) {
    extern "C" {
        fn ogx_SET_VARSIZE(ptr: *mut pg_sys::varlena, len: i32);
    }

    ogx_SET_VARSIZE(ptr, len)
}

pub unsafe fn set_varsize_short(ptr: *mut pg_sys::varlena, len: i32) {
    extern "C" {
        fn ogx_SET_VARSIZE_SHORT(ptr: *mut pg_sys::varlena, len: i32);
    }

    ogx_SET_VARSIZE_SHORT(ptr, len)
}
...
#[inline]
pub fn rust_str_to_text_p(s: &str) -> OgBox<pg_sys::varlena> {
    ...
    unsafe { OgBox::from_pg(bytea.as_ptr() as *mut pg_sys::varlena) }
}
...
#[inline]
pub fn rust_byte_slice_to_bytea(slice: &[u8]) -> OgBox<pg_sys::bytea> {
    // SAFETY:  `slice` will provide a valid pointer and pg_sys::cstring_to_text_with_len() will too
    unsafe {
        OgBox::from_pg(pg_sys::cstring_to_text_with_len(
