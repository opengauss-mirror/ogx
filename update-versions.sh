...
function update_main_files() {
    ...
    # Ignore all packages in ogx-examples/
    for file in ./ogx-examples/**/Cargo.toml; do
...
function update_extras() {
    ...
    CARGO_TOMLS_TO_SED=(
        ./Cargo.toml
        ./cargo-ogx/src/templates/cargo_toml
...
OGX_PG_SYS_GENERATE_BINDINGS_FOR_RELEASE=1 cargo test --no-run --workspace --no-default-features --features "og3"
